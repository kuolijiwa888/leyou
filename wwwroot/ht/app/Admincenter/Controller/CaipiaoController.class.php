...
class CaipiaoController extends CommonController {
	...
	function change_hemai_wanfa_type(){
		$cpname = I('cpname');
		$wanfa_type=I('wanfatype');
	...
	function fadan(){
		...
		$map['isopen'] = ['eq',1];
		$map['iswh'] = ['eq',0];
		$caipiaofadanlist = $this->_db->where($map)->order('allsort asc,id desc')->select();
		
		foreach($caipiaofadanlist as $k=>$v){
			$curr_wanfa_str = $v['hemai_wanfa_type'];
			$curr_wanfa_arr = explode(',',$curr_wanfa_str);
			$caipiaofadanlist[$k]['hemai_wanfa_type'] = $curr_wanfa_arr;
		}
		
		
		
		$this->fadan_caipiao_list = $caipiaofadanlist;
		//echo '<pre>';
		//var_dump($this->fadan_caipiao_list);die;
	...
	function change_hemai_status(){
		$cpname = I('cpname');
		$cp_hemai_status = I(cp_hemai_status);
		
		M('caipiao')->where(['name'=>$cpname])->data([
			'hemai_status' => $cp_hemai_status,
		])->save();
	...
	function change_hemai_value(){
		$cpname = I('cpname');
	
		$date = I();
		unset($date['cpname']);
		
		M('caipiao')->where(['name'=>$cpname])->data($date)->save();
	...
	function wanfa_odds(){
		$odds = $data = array();
		$typeid = I('get.typeid');
		if(empty($typeid)) $typeid = 'lhc';
		// if(!in_array($typeid,$types)) exit('error');
		if(IS_POST){
			
			$types = array('ssc','x5','k3','pk10','dpc','lhc','keno','dp3','xy28');
			if(!in_array($typeid,$types)) exit('error');
			$odds = I('post.odds');
			foreach ($odds as $key => $val) {
				...
				M('wanfa_odds')->where(array('playid'=>$key))->delete();
			}
			// var_dump($data);exit;
			M('wanfa_odds')->addAll($data);
		}
		$_wfobj = new \Lib\wanfa;
		// $wanfa = $_wfobj->$typeid();
		$wanfa = $_wfobj->$typeid();
		...
		$info = M('wanfa_odds')->where(array('typeid'=>$typeid))->select();
		foreach ($info as $vals) {
			$odds[$vals['playid']][$vals['fandian']] = $vals['odds'];
		}
		// var_dump($odds);exit;
		$this->assign(array(
			'wanfa' => $wanfa,
	...
	function wanfa_odds_ks(){
		$typeid = I('get.typeid');
		$fornum = 98;
		if(empty($typeid)) $typeid = 'lhc';
		//ssc,x5,k3,pk10,dpc,lhc,keno
		$types = array('ssc','x5','k3','pk10','dpc','lhc','keno','xy28');
		$odds = $data = array();
		if(!in_array($typeid,$types)) exit('error');
		if(IS_POST){
			...
			foreach ($odds as $key => $val) {
				...
				for ($i=0; $i <= $fornum; $i++) { 
					$odds_val = ($val[1]+($cz*$i));
					if($i==$fornum) $odds_val = $val[0];
					$data[] = array(
						'typeid' 	=> $typeid,
						'playid' 	=> $key,
						'fandian' 	=> $i/10,
						'odds' 		=> substr(sprintf("%.4f",$odds_val),0,-1),
					);
				}
				/*;
				foreach ($val as $ke => $va) {
	...
	/*function wanfa_odds(){
		$playid = I('get.playid');
		$wanfa = M('wanfa')->where(array('playid'=>$playid))->find();
		if(IS_POST){
			$odds = I('post.odds');
			$data = array();
			foreach ($odds as $key => $val) {
			...
			M('wanfa_odds')->where(array('playid'=>$playid))->delete();
			M('wanfa_odds')->addAll($data);
		}
		$info = M('wanfa_odds')->where(array('playid'=>$playid))->select();
		$data = array();
		foreach ($info as $vals) $data[$vals['fandian']] = $vals['odds'];
		$this->assign(array(
			'wanfa' => $wanfa,
			'data' => $data
		));
