	...
	function login(){
		...
		if(IS_POST){
			...
			if(!$param['name'] | !$param['pass'] | !$param['rzm']){
				echo jsonreturn(["sign"=>false,"message"=>"登录信息不完整"]);exit;
			}
			if(GetVar('islogincode')==1){
				$verify = new \Think\Verify(['reset' => false]);
				if(!$verify->check($param['code'])) {
				   echo jsonreturn(["sign"=>false,"message"=>"验证码错误"]);exit;
				}
			}
			if(GetVar('isemailcode')==1){
				...
				if(!$adminlogincode || time()-$adminlogincode['time']>intval(GetVar('adminemailcodetime'))){
					echo jsonreturn(["sign"=>false,"message"=>"邮件验证码不存在或已过期"]);exit;
				}
				if($adminlogincode['code']==$param['emailcode']){
					
				}else{
					echo jsonreturn(["sign"=>false,"message"=>"邮件验证码错误"]);exit;
				}
			...
			if(!$info){
				echo jsonreturn(["sign"=>false,"message"=>"管理员不存在"]);exit;
			}
			if($param['rzm']!=$info['safecode']){
				echo jsonreturn(["sign"=>false,"message"=>"安全码错误"]);exit;
			}
			...
			if($info['password']!=encrypt($param['pass'])){
				...
				echo jsonreturn(["sign"=>false,"message"=>"密码错误"]);exit;
			}
			if($info['islock']==1){
				echo jsonreturn(["sign"=>false,"message"=>"账户不允许登入"]);exit;
			}
			...
			if($admin_sessionid){
				...
				$obj = M('adminmember')->where(['id'=>$info['id']])->setField(['loginip'=>get_client_ip(),'iparea'=>$iparea,'logintime'=>NOW_TIME]);
 				//redirect(U('Index/index'));exit;
 				echo jsonreturn(["sign"=>true,"message"=>"登陆成功"]);exit;
			}else{
				echo jsonreturn(["sign"=>false,"message"=>"登录失败"]);exit;
			}
		}
