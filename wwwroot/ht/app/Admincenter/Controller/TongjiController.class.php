...
//		$where['isopen'] = 1;
	...
	function tdgaikuang(){
		...
		$Page       = new \Think\Page($count,$_pagasize);
		$show       = $Page->show();
		$list       = M('member')->where($map1)->order('id desc')->field('id,username,proxy')->limit($Page->firstRow.','.$Page->listRows)->select();
		foreach($list as $k=>$v){
			//获取所有下线用户
	...
	function cptouzhu3d(){
		...
		foreach($typelist as $k=>$v){
			...
			$piedata[] = "{value:".$v['amount'].",name:'".$v['title']."'}";
		}
