...
class MemberController extends CommonController {
	...
	function notice(){
		$list =D('Notice')->order('id desc')->select();
		$this->display();
	}
	...
	function manage(){
		...
		foreach($list as $k=>$v){
			...
			$list[$k] = $v;
			$list[$k]['yeb']=$v['yebmoney']+$v['dyebmoney'];
		}
...
//var_dump($list);die;
	...
	function useradd(){
		...
		if(IS_POST){
			...
			$_POST['is_rebet'] = 0;
			$fandian = I('fandian');
			$_POST['fandian'] = json_encode($fandian);
			$_POST['fandian'] = preg_replace('/^\{/', '', $_POST['fandian']);
			$_POST['fandian'] = preg_replace('/\}$/', '', $_POST['fandian']);
			...
			$_POST['regtime'] = time();
			$_POST['face'] = "/resources/images/face/".rand(1,40).".jpg";
			parent::_adddosimp();
		}
		$this->display();
	}
	function useradd1(){
		...
		if(IS_POST){
			...
			$password = 'dsfahlfhkda';
			$_POST['password'] = 'dsfahlfhkda';
			
			$tradepassword = 'dsfahlfhkda';
			$_POST['tradepassword'] = 'dsfahlfhkda';
...
//批量删除佣金数椐
	...
	function sameipuser(){
		...
		foreach($list as $k=>$v){
			if($v['count']>1){
				echo "<p>IP：<a href='".U('Member/manage')."?loginip={$v['loginip']}'>{$v['loginip']}</a>,会员：{$v['username']}(等),相同IP会员数量:{$v['count']}</p><hr>";
			}
		};
	}
	...
	function teamreport(){
		...
		$apiparam['startime'] = !empty($apiparam['startime'])?strtotime($apiparam['startime']):strtotime(date('Y-m-d 00:00:00',time()));
		$apiparam['endtime'] = !empty($apiparam['endtime'])?strtotime($apiparam['endtime']):strtotime(date('Y-m-d 00:00:00',time()))+86400;

		// $apiparam['loginname'] = 
		// var_dump(date('Y-m-d 00:00:00',time()));exit;

		$beginToday= mktime(0,0,0,date('m'),date('d'),date('Y'));
		$endToday  = mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
		$startime  = strtotime(date('Y-m-d 00:00:00',$apiparam['startime']));//时间戳
		$endtime   = strtotime(date('Y-m-d 23:59:59',$apiparam['endtime']));//时间戳
		$loginname = $apiparam['loginname'];
		
		if(!empty($loginname)){
			$map = array(
				'username' => ['eq',$loginname],
				'isnb' => ['eq',0],
			);
			$uid = M('member')->where($map)->order('id desc')->field('id')->select();
			// var_dump($uid);exit;
			$this->getdownlist($uid);
			$chiduids = $this->tree;
			$chiduids[] = $uid[0]['id'];
		}
		/*$map = [];
		...
		for($i=0;$i<=$days;$i++){
			...
			//自动充值
			$map = [];
			if(!empty($loginname))$map['uid'] = ['in',$chiduids];
