          ...
          <form class="layui-form toolbar" method="get" action="{:U(CONTROLLER_NAME.'/'.ACTION_NAME)}">
            <div class="layui-form-item">
              <div class="layui-inline">&emsp;
                <a class="layui-btn icon-btn" layer-url="{:U('add')}">
                  <i class="layui-icon">&#xe615;</i>添加会员组
                </a>&nbsp;
              </div>
            </div>
          </form>
            ...
            <table id="tbBasicTable" lay-filter="tbBasicTable">
              <thead>
                <tr>
                  ...
                  <th lay-data="{field:'username',sort:true,align: 'center'}">组名称</th>
                  <th lay-data="{field:'userbankname',sort:true,align: 'center'}">头衔</th>
              ...
              <tbody>
                ...
                <tr class="text-c">
                  ...
                  <td>{$vo.touhan}</td>
                  <td>
                    <u style="cursor:pointer" class="layui-btn layui-btn-primary layui-btn-xs" layer-url="{:U('setxiane',['id'=>$vo['groupid']])}" title="限额设置-{$vo.groupname}">限额设置</u>
                    <u style="cursor:pointer" class="layui-btn layui-btn-primary layui-btn-xs" layer-url="{:U('edit',['id'=>$vo['groupid']])}" title="修改-{$vo.groupname}">修改</u>
                    <u style="cursor:pointer" class="layui-btn layui-btn-danger layui-btn-xs" layer-del-url="{:U('delete',['id'=>$vo['groupid']])}" title="删除">删除</u>
                  </td>
                </tr>
                {/volist}
              </tbody>
