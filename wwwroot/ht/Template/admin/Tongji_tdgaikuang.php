        ...
        <div class="page-container">
          ...
          <form method="post" action="{:U(CONTROLLER_NAME.'/'.ACTION_NAME)}">
            <table id="tbBasicTable" lay-filter="tbBasicTable">
              <thead>
                <tr>
                  <th lay-data="{field:'id',sort:true,align: 'center',totalRowText: '页面小计'}">用户名</th>
                  <th lay-data="{field:'groupid',sort:true,align: 'center',totalRow: true}">总数</th>
                  <th lay-data="{field:'username',sort:true,align: 'center',totalRow: true}">代理数</th>
                  <th lay-data="{field:'userbankname',sort:true,align: 'center',totalRow: true}">会员数</th>
                  <th lay-data="{field:'shangji',sort:true,align: 'center',totalRow: true}">在线数</th>
                  <th lay-data="{field:'proxy',sort:true,align: 'center',totalRow: true}">自动充值</th>
                  <th lay-data="{field:'jinjijilu',sort:true,align: 'center',totalRow: true}">手动加款</th>
                  <th lay-data="{field:'balance',sort:true,align: 'center',totalRow: true}">手动减款</th>
                  <th lay-data="{field:'point',sort:true,align: 'center',totalRow: true}">提款</th>
                  <th lay-data="{field:'fandian',sort:true,align: 'center',totalRow: true}">充提盈亏</th>
                  <th lay-data="{field:'yeb',sort:true,align: 'center',totalRow: true}">投注</th>
                  <th lay-data="{field:'yeb1',sort:true,align: 'center',totalRow: true}">返奖</th>
                  <th lay-data="{field:'yeb2',sort:true,align: 'center',totalRow: true}">活动</th>
                  <th lay-data="{field:'yeb3',sort:true,align: 'center',totalRow: true}">投注盈亏</th>
                </tr> 
              </thead>
              <tbody>
                ...
                <tr class="text-c">
                  <td>
                    <?php if($vo['proxy'] == 1){ ?>
                    <a style="color:#5eb95e" href="Tongji.tdgaikuang.do?sDate={:I('sDate')}&eDate={:I('eDate')}&pid={$vo.id}">{$vo.username}</a>
                    <?php }else{ ?>
