            ...
            <table id="tbBasicTable" lay-filter="tbBasicTable">
              <thead>
                <tr>
                  ...
                  <th lay-data="{field:'username',sort:true,align: 'center'}">会员名称</th>
                  <th lay-data="{field:'username1',sort:true,align: 'center'}">晋级前等级</th>
                  <th lay-data="{field:'username2',sort:true,align: 'center'}">晋级后等级</th>
                  <th lay-data="{field:'username3',sort:true,align: 'center'}">晋级积分</th>
                  <th lay-data="{field:'userbankname1',sort:true,align: 'center'}">奖励金额</th>
                  <th lay-data="{field:'userbankname2',sort:true,align: 'center'}">时间</th>
                  <th lay-data="{field:'userbankname3',sort:true,align: 'center'}">操作</th>
                </tr> 
              </thead>
              <tbody>
                ...
                <tr class="text-c">
                  ...
                  <td>
                    ...
                    <u style="cursor:pointer;" class="layui-btn layui-btn-danger layui-btn-xs" layer-url="{:U('jinjishenhe',['id'=>$vo['id']])}" title="会员晋级审核">未审核</u>
                    {elseif condition="$vo['shenhe'] eq 1" /}
                    <u style="cursor:pointer;" class="layui-btn layui-btn-danger layui-btn-xs">审核通过</u>
                    {elseif condition="$vo['shenhe'] eq -1" /}
                    <u style="cursor:pointer;" class="layui-btn layui-btn-danger layui-btn-xs">退回</u>
                    {else /}
                    未知
                    {/if}
                    <u style="cursor:pointer" class="layui-btn layui-btn-danger layui-btn-xs" layer-del-url="{:U('jinjijianglidelete',array('id'=>$vo['id']))}" title="删除">删除</u>
                  </td>
                </tr>
                {/volist}
              </tbody>
