        ...
        <div class="page-container">
          ...
          <form class="layui-form toolbar" method="get" action="{:U(CONTROLLER_NAME.'/'.ACTION_NAME)}">
            <div class="layui-form-item">
              <div class="layui-inline">&emsp;
                <a class="layui-btn icon-btn" layer-url="{:U('add')}" type="submit" lay-filter="loginRecordTbSearch" lay-submit>
                  <i class="layui-icon">&#xe615;</i>添加管理组
                </a>&nbsp;
              </div>
            </div>
          </form>
          ...
          <form method="post" action="{:U(CONTROLLER_NAME.'/'.ACTION_NAME)}">
            <table id="tbBasicTable" lay-filter="tbBasicTable">
              <thead>
                <tr>
                  <th lay-data="{field:'groupid',sort:true,align: 'center'}">管理组</th>
                  <th lay-data="{field:'username',sort:true,align: 'center'}">管理用户名</th>
                  <th lay-data="{field:'username1',sort:true,align: 'center'}">状态</th>
                  <th lay-data="{field:'userbankname4',sort:true,align: 'center'}">操作</th>
                </tr> 
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <u lock-url="{:U('lock',['id'=>$vo['id']])}" value="{if condition='$vo[islock] eq 0'}正常{elseif condition='$vo[islock] eq 1' /}锁定{/if}">
                      <input type="checkbox" class="userId" lay-filter="userTbStateCk"  lay-skin="switch" lay-text="正常|锁定" {if condition='$vo[islock] eq 0'}checked{elseif condition='$vo[islock] eq 0' /}{/if}/>
                    </u>
                  </td>
                  <td>
                    <u style="cursor:pointer" class="layui-btn layui-btn-primary layui-btn-xs" layer-url="{:U('edit',['id'=>$vo['id']])}" title="修改-{$vo.username}">修改</u> 
                    <u style="cursor:pointer" class="layui-btn layui-btn-danger layui-btn-xs" layer-del-url="{:U('delete',array('id'=>$vo['id']))}" title="删除">删除</u>
                  </td>
                </tr>
                {/volist}
              </tbody>
