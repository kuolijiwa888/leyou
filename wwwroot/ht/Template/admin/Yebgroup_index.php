						...
						<table id="tbBasicTable" lay-filter="tbBasicTable">
							<thead>
								<tr>
									...
									<th lay-data="{field:'username1',sort:true,align: 'center'}">名称</th>
									<th lay-data="{field:'username2',sort:true,align: 'center'}">天数</th>
									<th lay-data="{field:'username3',sort:true,align: 'center'}">利润</th>
									<th lay-data="{field:'username4',sort:true,align: 'center'}">状态</th>
							...
							<tbody>
								...
								<tr>
									...
									<td>{$vo.day}</td>
									<td>{$vo.liyun}</td>
									<td>{if condition="$vo['state'] eq 1"}<span class="c-green">可用</span>{else /}<span class="c-999">禁用</span>{/if}</td>
									<td>
										<u style="cursor:pointer" class="layui-btn layui-btn-primary layui-btn-xs" layer-url="{:U('edit',['id'=>$vo['id']])}" title="修改-{$vo.groupname}">修改</u> 
