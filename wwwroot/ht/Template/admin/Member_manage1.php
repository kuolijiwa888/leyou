					...
					<form class="layui-form toolbar" method="get" action="{:U(CONTROLLER_NAME.'/'.ACTION_NAME)}">
						<div class="layui-form-item">
							...
							<div class="layui-inline">&emsp;
								<a class="layui-btn icon-btn" layer-url="{:U('useradd',['parentid'=>$parentid])}" type="submit" lay-filter="loginRecordTbSearch" lay-submit>
									<i class="layui-icon">&#xe615;</i>添加机器人
								</a>&nbsp;
							</div>
							{else /}
							<div class="layui-inline">&emsp;
								<a class="layui-btn icon-btn" layer-url="{:U('useradd1')}" type="submit" lay-filter="loginRecordTbSearch" lay-submit>
									<i class="layui-icon">&#xe615;</i>添加机器人
								</a>&nbsp;
							</div>
							{/present}
						...
						<table id="tbBasicTable" lay-filter="tbBasicTable">
							...
							<tbody>
								...
								<tr>
									<td>{$vo.username}</td>
									<td>
										<u style="cursor:pointer" class="layui-btn layui-btn-danger layui-btn-xs" layer-del-url="{:U('userdelete',['id'=>$vo['id']])}" title="删除-{$vo.username}">删除</u>
									</td>
								</tr>
								{/volist}
							</tbody>
