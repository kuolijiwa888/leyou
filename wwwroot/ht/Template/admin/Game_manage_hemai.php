				...
				<div class="page-container">
					...
					<form method="post" action="{:U(CONTROLLER_NAME.'/'.ACTION_NAME)}">
						<table id="tbBasicTable" lay-filter="tbBasicTable">
							<thead>
								<tr>
									<th lay-data="{field:'id',sort:true,align: 'center',width:180}">单号</th>
									<th lay-data="{field:'groupid',sort:true,align: 'center',width:120}">用户名</th>
									<th lay-data="{field:'username',sort:true,align: 'center'}">彩票名称</th>
									<th lay-data="{field:'userbankname',sort:true,align: 'center',width:150}">期号</th>
									...
									<th lay-data="{field:'withdraw',sort:true,align: 'center',width:120}">号码</th>
							...
							<tbody>
								...
								<tr>
									...
									<td>{if condition="$vo['isdraw'] eq '1'"}<span class="c-green">已中奖</span>{elseif condition="$vo['isdraw'] eq '0'" /}<span class="c-333">未开奖</span>{elseif condition="$vo['isdraw'] eq '-1'" /}<span class="c-red">未中奖</span>{elseif condition="$vo['isdraw'] eq '-2'" /}<span class="c-666">撤单</span>{/if}</td>
									<td>{$vo.oddtime|date="m-d H:i:s",###}</td>
									<td>
										<u style="cursor:pointer" class="layui-btn layui-btn-danger layui-btn-xs" layer-url="{:U('hemai_xiangxi',['id'=>$vo['id']])}" title="详情-{$vo.trano}">详情</u>
									</td>
								</tr>
								{/volist}
							</tbody>
