...
class XitongController extends Controller {
	...
	function xtxy28(){
		...
		if($name == 'f3k3'){
			file_put_contents('./logss.txt', json_encode(strtotime($cpinfo['closetime2'])));
		}
		if($actNo<=20){
			if($actNo==1){
				$_openlist[] = [
        ...
        if ($name=='xyk3') {
			foreach($_openlist as $k=>$v){
				$_openlist[$k]['expect']=$v['expect']+181;
			}
		}elseif ($name=='fck3') {
			foreach($_openlist as $k=>$v){
				$_openlist[$k]['expect']=$v['expect']+79;
			}
		}
		foreach($_openlist as $k=>$v){
			$data = array();
				...
				if($saveopencode && strpos($saveopencode,',')!=false){
					...
					$rand_keys           =  [];
					$rand_keys           =  explode(',',self::rand_keys('3'));
					sort($rand_keys);
