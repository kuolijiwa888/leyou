...
<body>
	...
	<div class="Recharge-top">
		<ul>
			<li><a href="/userCenter/billRecord">全部</a></li>
			<li><a href="/userCenter/billRecord1">充值</a></li>
			<li><a class="Recharge-active" href="/userCenter/billRecord2">提现</a></li>
		</ul>
	</div>
	...
	<div class="Recharge-game" style="margin-bottom: .78rem;">
		<ul>
			<volist name="mxlist" id="vo">
				<li <if condition="$vo['state'] eq 1">class="tongguo"</if><if condition="$vo['state'] eq -1">class="guanbi"</if>>
					<div class="Recharge-game-top">
						<i class="iconfont">&#xe653;</i>
						<span>{$vo.bankname}</span>
						<span style="float:right;">{$vo.oddtime|date="Y-m-d H:i:s",###}
						</span>
					</div>
					<div class="Recharge-game-bottom">
						<div class="Recharge-qihao"><p>编号</p><em>{$vo.trano}</em></div>
						<div class="Recharge-jine"><p>金额</p><em>{$vo.amount}</em></div>
						<div class="Recharge-zhuangt"><em><if condition="$vo['state'] eq 1">提款成功</if><if condition="$vo['state'] eq 0">处理中</if><if condition="$vo['state'] eq -1">提款退回</if></em></div>
					</div>
				</li>
			</volist>
		</ul>
	...
	<script>
		...
		function getlist(atime){
			window.location.href="/userCenter/Account3/atime/"+atime;
		}
	</script>
</body>
</html>