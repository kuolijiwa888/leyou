...
<body>
	...
	<div class="gaimi">
		<form action="/userbankname" method="post" onsubmit="return checkform(this)">
			<div class="gaimi-top">
				<ul>
					<li>
						<div class="gaimi-top-name">真实姓名:</div>
						<div class="gaimi-top-mima"><input type="text" placeholder="请输入真实姓名" id="username"  name="userbankname"></div>
					</li>
					<li>
						<div class="gaimi-top-name">资金密码:</div>
						<div class="gaimi-top-mima"><input type="password" placeholder="请输入资金密码" name="tradepassword"></div>
					</li>
				</ul>
				<div class="gaimi-top-button"><button type="submit" onclick="return IsChinese();">确定</button></div>
			</div>
		</form>
	</div>
	...
	<script>
		function IsChinese()
		{
			...
			if(str.length!=0){
				reg=/^[\u0391-\uFFE5]+$/;
				if(!reg.test(str)){
					hsycmserror("请输入真实中文姓名");//请将“字符串类型”要换成你要验证的那个属性名称！
					return false;
				}
			}
			if(str.length<1){
				hsycmserror("请输入真实中文姓名");
				return false;
			}
		}
	</script>
		...
		function checkform(obj){
			$.post($(obj).attr('action'),$(obj).serialize(), function(json){
				if(json.status==1){
					...
					setTimeout(function(){
				    window.location.href = "/memberCenter/securityCenter";
			        },2000); 
				}else{
