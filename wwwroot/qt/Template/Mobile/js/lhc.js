	...
	$node.html('');
	  $.post(url, {'lotteryname':lotteryname}, function(data) {
        ...
        var yopentime = yopentimes_arr[1];
				html += '<div class="lotterys"><div class="rq">第 '+openinfo.expect+' 期</div><div class="hm">'+lhckaijList+'</div></div>';
...
function lhcMobileKaiHtml(Numarr){
	...
	for(var i = 0; i < Numarr.length; i++){
		...
		console.log(typeof lhcNumberData);
		if(typeof lhcNumberData == "object"){
			if(i == Numarr.length - 1){
				html += '<div>+</div>';
			}	
			html += '<div><span class="qiu lhc_'+Numarr[i]+'">'+Numarr[i]+'</span><em>'+lhcNumberData.sx+'</em></div>';	
		}
