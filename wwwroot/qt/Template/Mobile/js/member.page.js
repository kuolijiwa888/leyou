(function($) {
	$.pagination = function(options) {
		...
		var update = function(totalCount) {
			...
			//infos.append(infosP);
			var pages = $('<div class="fanye">');
			pages.append($('<div>').html($('<a class="prev" href="javascript:;">').html(language.prev)));
			var topPage = $('<div>').html($('<a class="start" href="javascript:;">').html(1));
			if(page == 1) {
			...
			if(pageCount > 2){
				...
				if(startPage == 1) {
					...
					pages.append($('<div>').html('...'));
				}
				if(endPage == pageCount) {
					endPage--;
				}
				for (var i = startPage; i <= endPage; i++) {
					var thisPage = $('<div>').html($('<a class="apage" href="javascript:;">').html(i));
					if(i == page) {
			...
			if(pageCount >= 2) {
				var endPages = $('<div>').html($('<a class="end5" href="javascript:;">').html(pageCount));
				if(page == pageCount) {
					endPages.addClass('cur');
				}
				pages.append(endPages);
			}
			pages.append($('<div>').html($('<a class="next" href="javascript:;">').html(language.next)));
			...
			pages.find('.end5').click(function() {
				if(page < pageCount) {
					page = pageCount;
					load();
				}
