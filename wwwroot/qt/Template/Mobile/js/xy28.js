...
var lotterytimes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname,'cptype':'xy28'}, function(data) {
		...
		if(data.sign==true){
			lottery = data.data;
			//lottery[0].expect = Number(lottery[0].expect)-1;
	...
	$node.html('');
	  $.post(url, {'lotteryname':lotteryname}, function(data) {
        ...
        var yopentime = yopentimes_arr[1];
				...
				html += '<div class="lotterys"><div class="rq">第 '+openinfo.expect+' 期</div><div class="hm"><span class="qiu">'+array[0]+'</span><span class="qiu">'+array[1]+'</span><span class="qiu">'+array[2]+'</span></div></div>';
...
function lhcMobileKaiHtml(Numarr){
	...
	for(var i = 0; i < Numarr.length; i++){
		lhcNumberData = eval('xy28_numData.xy28_'+Numarr[i]);

		if(typeof lhcNumberData == "object"){
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
                        ...
                        var yopentimes_arr = yopentimes.split(' ');
                        var yopentime = yopentimes_arr[1];
                        var html = '<div class="lotterys"><div class="rq">第 ' + openexpect + ' 期</div><div class="hm"><span class="qiu">'+openCodes[0]+'</span><span class="qiu">'+openCodes[1]+'</span><span class="qiu">'+openCodes[2]+'</span></div></div>';
...
function setLotterNumber(_index,_nums) {
  ...
  if(_nums == 'gif'){
    ...
    $('#lhc_winning_sum').find('li').css('text-indent','-9999px');
  	...
  	//if(_nums.length < 2) _nums = '0'+_nums;


		var lhcNumberData = eval('xy28_numData.xy28_'+_nums);
