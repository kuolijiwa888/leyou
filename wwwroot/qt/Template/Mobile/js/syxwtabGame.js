...
function syxwqezxbd(){
  ...
  $(document).on('click','.bet_options',function (){
    ...
    switch(_thisType){
      ...
      case 'x5dds':
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从6种单双个数组合中选择1种组合，当开奖号码的单双个数与所选单双组合一致   <span class="mobileMoneyInfo" style="color: #a68f4c;">奖金详情</span>');
...
function xselectRmb(moneyStr,selectRmb) {
  ...
  $(document).on("click", ".hemai", function() {
    ...
    if(zhushus.length >= 1){
      ...
      //console.log(orderList);
      var jine = $('.selectMultipleOldMoney').text();
      $('.selectMultipleOldMoney').text(jine);
      $('.g_Number_Section').find('a').removeClass('curr');
      $('#text').val('');
      currNumber = [];
      zhushus = [];
 ...
 $('.noe').click(function(){
  ...
  $(document).on('click','#j_play_select .play_select_tit li',function () {
    ...
    switch(this_attr){
      case 'sm':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box1'),syxw_sm_title,syxw_sm);
      _thisPlayid = 'x5qsfs';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      ...
      case 'em':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box2'),syxw_em_title,syxw_em);
      _thisPlayid = 'x5qefs';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从01-11共11个号码中选择2个不重复的号码组成一注，所选号码与当期顺序摇出的5个号码中 的前2个号码相同，且顺序一致，即为中奖，奖金 <em style="color:red;">'+rates.maxjj+'</em>元');
      ...
      case 'bdw':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box3'),syxw_qsymbdw_title,syxw_qsymbdw);
      _thisPlayid = 'x5bdwqs';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从01-11共11个号码中选择1个号码，每注由1个号码组成，只要当期顺序摇出的第一位、第二位、第三位开奖号码中包含所选号码，奖金 <em style="color:red;">'+rates.maxjj+'</em>元');
      ...
      case 'dwd':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box4'),syxw_dwd_title,syxw_dwd);
      _thisPlayid = 'x5dwd';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从第一位、第二位、第三位任意1个位置或多个位置上选择1个号码，所选号码与相同位置上的开奖号码一致，奖金 <em style="color:red;">'+rates.maxjj+'</em>元');
      ...
      case 'qw':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box5'),syxw_qwx_title,syxw_qwx);
      _thisPlayid = 'x5dds';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从6种单双个数组合中选择1种组合，当开奖号码的单双个数与所选单双组合一致 <span class="mobileMoneyInfo" style="color: #a68f4c;">奖金详情</span>');
      ...
      case 'rxfs':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box6'),syxw_rxfs_title,syxw_rxfs);
      _thisPlayid = 'x5rx1z1';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从01-11共11个号码中选择1个号码进行购买，当期的5个开奖号码中包含所选号码，奖金  <em style="color:red;">'+rates.maxjj+'</em>元');
      ...
      case 'rxds':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box7'),syxw_rxds_title,syxw_rxds);
      ...
      case 'rxtd':
      $('#bet_filter').remove();
      gameSwitch($('.bet_filter_box8'),syxw_rxtd_title,syxw_rxtd);
      _thisPlayid = 'x5dt2z2';
      rates = yrates[_thisPlayid];
      $('.play_select_prompt').find('span[way-data="tabDoc"]')
      .html('从01-11共11个号码中至少选择2个以上号码进行投注，每注需至少包括1个胆码及1个拖码。只要当期的5个开奖号码中有2个包含所选号码（每注包含2个号码），即为中奖，奖金 <em style="color:red;">'+rates.maxjj+'</em>元');
