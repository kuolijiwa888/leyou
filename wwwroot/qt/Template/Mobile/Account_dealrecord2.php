...
<body>
	...
	<div class="Recharge-top">
		<ul>
			...
			<li><a class="Recharge-active">充值</a></li>
			...
			<volist name="mxlist" id="vo">
				<li <if condition="$vo['state'] eq 1">class="tongguo"</if><if condition="$vo['state'] eq -1">class="guanbi"</if>>
					...
					<div class="Recharge-game-bottom">
						...
						<div class="Recharge-zhuangt"><em><if condition="$vo['state'] eq 1">充值成功</if><if condition="$vo['state'] eq 0">支付中</if><if condition="$vo['state'] eq -1">订单关闭</if></em></div>
					</div>
				</li>
			</volist>
		</ul>
	...
	<script>
		...
		function getlist(atime){
			window.location.href="/userCenter/Account2/atime/"+atime;
		}
	</script>
</body>
</html>