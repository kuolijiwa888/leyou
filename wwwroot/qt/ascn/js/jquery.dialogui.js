(function(g) {
	...
	g.blockUI_lang = {
		...
		img_dir: "./js/web/dialogUI/"
	};
	g.blockUI = function(o) {
		$(".eycinfoClose").trigger("click");
		e(window, o);
	};
	...
	g.alert = function(t, r, q, p, isSureBtn) {
		...
		var o = m({
			...
			title: r,
			msg: t,
			//bt_text: '<div class="eycpopupwindowMidBtn confirmStyle" id="alert_close_button">' + g.blockUI_lang.button_sure + '</div>'
			bt_text: !isSureBtn ? '' : '<input class="eycpopupwindowMidBtn confirmStyle" id="alert_close_button" type="button" value="'+g.blockUI_lang.button_sure+'">'
		});
		$html = g(o);
		g.blockUI({
			message: $html,
        ...
        g("#block_close", $html).add(g("#closeButton", $html)).click(function() {
            g.unblockUI({
                fadeInTime: 0,
                fadeOutTime: 0,
                onUnblock: q
	...
	g.confirm = function(u, o, r, t, q, lft, rit) {
		t = (t && t.length > 1) ? t: g.blockUI_lang.title_confirm;
		lft = (lft && lft.length >1) ? lft : g.blockUI_lang.button_sure;
		rit = (rit && rit.length >1) ? rit : g.blockUI_lang.button_cancel;
		u = u ? u: undefined;
		...
		var bttext = (rit!=g.blockUI_lang.button_cancel)?('<input class="eycpopupwindowMidBtn confirmStyle" id="confirm_yes" type="button" value="'+lft+'"><input class="eycpopupwindowMidBtn confirmStyle" id="confirm_no" type="button" value="'+rit+'">'):('<input class="eycpopupwindowMidBtn confirmStyle" id="confirm_yes" type="button" value="'+lft+'"><input class="eycpopupwindowMidBtn cancelStyle" id="confirm_no" type="button" value="'+rit+'">');
		var p = m({
	...
	g.ajaxUploadUI = function(p) {
		...
		var v = m({
			...
			title: p.title,
			msg: z,
            bt_text:'<div class="eycpopupwindowMidBtn confirmStyle" id="block_ajaxConfirm">' + g.blockUI_lang.button_upload + '</div><div class="eycpopupwindowMidBtn cancelStyle" id="block_close">' + g.blockUI_lang.button_cancel + '</div>'
		});
		$html = g(v);
		g.blockUI({
			message: $html,
	...
	function m(p) {
		...
		p = g.extend({},
		q, p || {});
        var o = '<span class="eycpopupwindow"><div class="eycpopupwindowHeader"><span>提示</span><div class="eycpopupuwindowClose" id="closeButton"></div></div><div class="eycpopupwindowMid"><div class="eycpopupwindowMidText"><img src="../../../images/web/betnew/tips.png" style="display: inline-block;position: relative;top: 6px;right: 2px;" alt="">'+p.msg+'</div><div class="eycpopupwindowMidBtnCenter">'+p.bt_text+'</div></div></span>';
		return o
	}
	g.blockUI.defaults = {
	...
	function e(q, o) {
		...
		var r = C ? g('<div id="JS_blockPage" style="z-index:' + D + ';position:fixed"></div>') : g('<div  id="JS_blockElement" style="z-index:' + D + ';position:fixed"></div>');
