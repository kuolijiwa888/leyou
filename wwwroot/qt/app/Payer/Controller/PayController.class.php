      ...
      //     }
		...
		$configs = $bankpaylists[$paytype]["configs"];
			...
			case 'ipaybank':
				...
				echo $returnjump;
				break;
			case 'bjalipay': case 'bjh5alipay': case 'bjweixin': case 'bjqqpay': case 'bjwy':
				F($payinfo['trano'],$payinfo,'./PayOrder/');
				require("./ipay/bjpay/qsclass.php");
				...
				$notifyUrl = $configs['returnbackurl'].'/Payback.bjpaybank';//异步通知的地址
				$returnUrl = $host.'/Member.orderform?tabid=rechargelist';//前台地址
				$parameter = array(
                    "type"          => $PaymentTypeCode[$payinfo['paytype']],
                    "notify_url"    => $notifyUrl,
                    "return_url"    => $returnUrl,
                    "out_trade_no"  => $payinfo['trano'],
                    "subject"       => '',
			...
			case 'rjwy':
				...
				break;
			/*case 'ofbalipay': case 'ofbweixin': case 'ofbwy':
				F($payinfo['trano'],$payinfo,'./PayOrder/');
				require("./ipay/ofbpay/qsclass.php");
				$PaymentTypeCode = array(
					'ofbalipay' => '174',
					'ofbweixin' => '131',
					'ofbwy'  	=> '178'
				);
				$notifyUrl = $configs['returnbackurl'].'/Payback.ofbpaybank';//异步通知的地址
				$returnUrl = $host.'/Member.orderform?tabid=rechargelist';//前台地址
				$parameter = array(
                    "bankcode"          => $PaymentTypeCode[$payinfo['paytype']],
                    "notifyurl"    		=> $notifyUrl,
                    "returnurl"    		=> $returnUrl,
                    "remark"  			=> $payinfo['trano'],
                    ...
                    "product" 			=> 'payin',
                    "orderid" 			=> $osn = date('Ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8).'1234'
                );
                $qsclass = new \qsclass();
                $html = $qsclass->buildRequestForm($parameter);
