...
//公用
	public function gameHeader(){
      ...
      //$type=I('post.type');
      //$lottery_num = M('canglottery')->where("uid=".$this->userinfo['id'])->getfield('status');
		$data['uid'] = $this->userinfo['id'];
		//$data['status'] = 1;
		$lottery_num = M('canglottery')->where($data)->select();
		$this->assign('lottery_num',$lottery_num);
      //$this->lottery_num = $lottery_num;
		$gamecplist = S('gamecplist');
		$gamecplist = false;
		if(!$gamecplist){
			$_allcp = M('Caipiao')->field('id,title,typeid,name,isct')->where("isopen='1' AND iswh='0'")->order('listorder ASC')->select();
		}else{
			$_allcp = $gamecplist;
		}
...
//K3
	function k3(){
		...
		// $_k3list = C('cplist.k3');
		$_k3list = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='k3'")->order('listorder ASC')->select();
		foreach($_k3list as $k=>$v){
			if($v['isopen']==0){
				// print_r($v['isopen']);exit();
			}
			
		}
		 // print_r($_k3list);exit();
		...
		if($isct){
			if ($lotteryname=='jisk3') {
				$this->display("Game_jisk3_ct");
			}else{
				$this->display("Game_k3_ct");
			}
		}elseif ($lotteryname=='jisk3') {
			$this->display('Game_jisk3');
		}
		else{
			$this->display();
		}
...
//SSC
	function ssc(){
		...
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='ssc'")->order('listorder ASC')->select();
		$other = $Allcp = $hotcaipiao = M("caipiao")->where("isopen = 1")->cache(300)->order('allsort ASC')->select();
	...
	function pk10(){
		...
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='pk10'")->order('listorder ASC')->select();
		foreach($_ssclist as $k=>$v){
			if($v['isopen']==0)unset($_ssclist[$k]);
		}
		$this->assign('ssclist',$_ssclist);
	...
	function keno(){
		...
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='keno'")->order('listorder ASC')->select();
		foreach($_ssclist as $k=>$v){
			if($v['isopen']==0)unset($_ssclist[$k]);
		}
		$this->assign('ssclist',$_ssclist);
	...
	function x5(){
		...
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='x5'")->order('listorder ASC')->select();
		foreach($_ssclist as $k=>$v){
			if($v['isopen']==0)unset($_ssclist[$k]);
		}
		$this->assign('ssclist',$_ssclist);
	...
	function dpc(){
		...
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='dpc'")->order('listorder ASC')->select();
		foreach($_ssclist as $k=>$v){
			if($v['isopen']==0)unset($_ssclist[$k]);
		}
		$this->assign('ssclist',$_ssclist);
	...
	function lhc(){
		...
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='lhc'")->order('listorder ASC')->select();
		foreach($_ssclist as $k=>$v){
			if($v['isopen']==0)unset($_ssclist[$k]);
		}
		$this->assign('ssclist',$_ssclist);
	...
	function xy28(){
		...
		$lotteryname = I('code');
		$this->assign('lotteryname',$lotteryname);
		$_ssclist = C('cplist.xy28');
		$_ssclist = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='xy28'")->order('listorder ASC')->select();
		foreach($_ssclist as $k=>$v){
			if($v['isopen']==0)unset($_ssclist[$k]);
		}
		$this->assign('ssclist',$_ssclist);
						...
						if($userinfo['fandian']){
							if(strstr($v0['maxjj'],'|')){
								...
								foreach($v01 as $j=>$v){
									...
									$maxjjstr .= sprintf("%.3f", filter_money($maxstr,2)).'|';
								}
								...
								/*if(substr(explode('.',$maxjj)[1],0,2)=='99'){
									$Result['data'][$v0['playid']]['maxjj']=sprintf("%.3f", ceil($maxjj));
								}else{
									$Result['data'][$v0['playid']]['maxjj'] =sprintf("%.3f", filter_money($maxjj,2));
								}*/
							}
						}
						$Result['data'][$v0['playid']]['minxf'] = $groupinfo['min_'.$v0['playid']]?$groupinfo['min_'.$v0['playid']]:($Result['data'][$v0['playid']]['minxf']?$Result['data'][$v0['playid']]['minxf']:0);
	...
	//玩法说明
		...
		function details(){			
			...
			$no = I('no');
			$map['id'] = ['eq',$no];
			$result=M('touzhu')->where($map)->select();
			$maphm['trano'] = ['eq',$result[0]['trano']];
			$resulthm=M('touzhuhm')->where($maphm)->select();		
			$mapfind['username'] = ['eq',$this->userinfo['username']];
			$mapfind['trano']=['eq',$result[0]['trano']];		
			$resultfind=M('touzhuhm')->where($mapfind)->find();
			...
			$mapcp['name'] = ['eq',$result[0]['cpname']];
			$cp = M("caipiao")->where($mapcp)->cache(300)->find();
			$mapface['username'] = ['eq',$result[0]['username']];
			$face=M('member')->where($mapface)->select();
			$this->assign('cp',$cp);
			$this->assign('face',$face);
			$this->assign('inlist',empty($resultfind)?'true':'false');	
			$this->assign('resulthm',$resulthm);
			$this->assign('isstop',$isstop);
			$this->assign('result',$result);
			$this->display();
		}
		
		private function isfull($result){
