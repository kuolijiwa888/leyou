...
//			$minrecharge = floatval(GetVar('minrecharge'))>0?floatval(GetVar('minrecharge')):50;
    ...
    function  withdrawals()
    {
        ...
        if(empty($_banklist))
        {
            ...
            //redirect("/userCenter/withdraw");exit;
		...
		$this->assign('tikuanstart',$tikuanstart);
		$this->assign('tikuanend',$tikuanend);
        $this->display();
    }
    ...
    function  refreshmoney(){
        $yebmoney= M('member')->where("id=".$this->userinfo['id'])->getField('yebmoney');
		$dyebmoney= M('member')->where("id=".$this->userinfo['id'])->getField('dyebmoney');
		$yeblixi= M('member')->where("id=".$this->userinfo['id'])->getField('yeblixi');
		$yeball = $yebmoney + $dyebmoney + $yeblixi;
        $money = M('member')->where("id='{$this->userinfo['id']}'")->find();
        $ascn = array('balance'=>$money['balance'],'xima'=>$money['xima'],'yeball'=>$yeball,'ngbalance'=>$money['ngbalance'],'point'=>$money['point']);
        echo jsonreturn($ascn);
    }
    //提现记录
    ...
    function zfbpay()
    {
        $Allmsg = M('payset')->field("paytype ,paytypetitle,configs ,minmoney,maxmoney,remark,state")->where("isonline=-1  AND paytype='alipay'")->select();
    ...
    function mazhifu()
    {
        $Allmsg = M('payset')->field("paytype ,paytypetitle,configs ,minmoney,maxmoney,remark,state")->where("isonline=1  AND paytype='mazhifu'")->select();
        foreach ($Allmsg as $key => $value)
        {
            $Allmsg['paytype']  =  $value['paytype'];
            $Allmsg['paytypetitle']  =  $value['paytypetitle'];
    ...
    function wxRecharge()
    {
        $Allmsg = M('payset')->field("paytype ,paytypetitle,configs ,minmoney,maxmoney,remark,state")->where("isonline=-1 AND paytype='weixin'")->select();
        foreach ($Allmsg as $key => $value)
        {
            $Allmsg['paytype']  =  $value['paytype'];
            $Allmsg['paytypetitle']  =  $value['paytypetitle'];
    ...
    function qttzRecord()
    {
        ...
        if(empty($order_sn)){
            ...
            $notify_url=$paysxx['returnbackurl'];
            $return_url ='http://'.$_SERVER['HTTP_HOST'].'/'.$paysxx['hrefbackurl'];
            $signa = md5(md5($money.$notify_url.$order_sn).$paysxx['merchantkey1']);
            $param = array (
                'notify_url' =>$notify_url,
                'return_url' =>$return_url,
                'order_sn' => $order_sn,//商户订单号
                'money' => $money,//商户订单金额
                'user_id' => $paysxx['merchantid'],//商户会员ID
                'sign' => $signa,//签名
            );

            $sHtml = '';
            $sHtml .= "<h3>正在跳转到支付....</h3>";
            $sHtml .= "<form id='cbjpaysubmit' name='cbjpaysubmit' action='https://pay.shengtuo8.com/api/pal/post' method='POST'>";
