...
//        dump($_SESSION);
		...
		// $other = $Allcp = $hotcaipiao = M("caipiao")->where("isopen = 1")->where(array('typeid'=>array('IN',array('k3','pk10'))))->cache(300)->order('allsort ASC')->select();
...
// 单个彩种排序
	...
	public function home(){
	    $address = M('address')->where('is_mobile=0')->select();
        $this->assign('address',$address);
        
        $gonggao = M('gonggao')->limit(4)->order('id DESC')->select();
...
// 单个彩种排序
	...
	function ht(){
		...
		// $odds = $oddss;
		// var_dump($odds['bjpk10qian1']['9.8']);exit;
		$this->assign(array(
			'wanfa' => $wanfa,
			'odds' => $odds,
			'typeid' => $typeid
