...
class ApijiekouController extends \Common\Controller\ApijiekouController {
	...
	function getpayhost(){
		...
		if($paytype == 'fourinone'){
			
			$accountinfo = M('payste')->where("paytype='fourinone'")->find();
	...
	function getLottery(){
		...
		$cptype = I("cptype");
		$_cplist = array_merge(M('caipiao')->where("isopen=1")->order('allsort asc,id desc')->select());
	...
	function lotterytimes(){
		...
		$lotteryname = I('lotteryname');
		$cptype = I("cptype");
		// var_dump($cptype);exit;
		// $cplist = M('caipiao')->where(['isopen'=>1])->cache(0,true)->field('name')->select();
		// print_r($cplist);
		switch ($cptype){
			case "ssc" :
				$cplist = C('cplist.ssc');
	 ...
	 // print_r($Data);
		...
		if($_lottetimes['lastFullExpect'] && $lotteryname!='jisk3'){
			...
			$apiparam['sign'] = true;
			$apiparam['message'] = '获取成功';
			$toulist = M('touzhu')->where("cpname='jisk3' and uid='{$uid}'")->order('id desc')->limit(1)->find();
			$date=strtotime(date("Y-m-d 00:00:00"));
			
			$nowtous = M('touzhu')->where("cpname='jisk3' and oddtime >= '{$date}' and uid='{$uid}' ")->limit(1)->find();
			if (empty($toulist)) {
				$return['lastFullExpect']  = 0;			
			    $return['lastExpect'] = 0;
			}else{
                 $return['lastFullExpect']  = $toulist['expect'];			
			    $return['lastExpect'] = substr($toulist['expect'],8);
			}
			
			if (empty($nowtous['expect'])) {
				$return['currFullExpect'] = date('Ymd',time()).'001';
			}else{
				$return['currFullExpect'] = $toulist['expect']+1;
			}
			
			$return['shortname']  ='极速快三';
			$return['currExpect']   = substr($return['currFullExpect'],8);
			$apiparam['data'] = $return;
			echo jsonreturn($apiparam);exit;
...
//		dump($Result);exit;
	...
	public function lotteryopencodes(){
/*		if(!IS_POST){
			echo jsonreturn(['sign'=>false,'message'=>'非法操作']);exit;
		}*/
		$lotteryname = I('lotteryname');
...
//		exit(dump($Result));
	...
	function hmadd(){
		...
		$sessionid = session('member_sessionid');
		$auth_id   = session('member_auth_id');
		if(!$_REQUEST['action'] || !$_REQUEST['pid'] || !$_REQUEST['buynum'] || !$_REQUEST['onemoney']){
			$this->ajaxReturn(['sign'=>false,'message'=>'缺少彩种标识或投注期号']);exit;
		}		
		$userinfo=session('userinfo');
		if(!$sessionid || !$auth_id || !$userinfo){
		...
		$apiparam=array();
		$apiparam['action'] = $_REQUEST['action'];
		$apiparam['pid'] = $_REQUEST['pid'];
		$apiparam['buynum']=$_REQUEST['buynum'];
		$apiparam['onemoney']=$_REQUEST['onemoney'];
		$apiparam['auth'] = ["member_auth_id"=>$auth_id,"member_sessionid"=>$sessionid];		
		$_api = new \Lib\api;
		$Result = [];$Result = $_api->sendHttpClient('Api/Lottery/hmadd',$apiparam);
		$this->ajaxReturn($Result);		
	}
	
	public function get_zero_or_one(){
		$res =  mt_rand(0,1);
		return $res;
	}
	
	
	function zidongfadan_all(){
		$cplist = M('caipiao')->where(['isopen'=>1,'iswh'=>0,'hemai_status'=>1])->select();
		foreach($cplist as $cp){
			
			if($cp['typeid'] == 'k3'){
				$this->zidongfadan_k3($cp['name']);
				//echo $cp['name'];
			}
			
			if($cp['typeid'] == 'ssc'){
				$this->zidongfadan_ssc($cp['name']);
				//echo $cp['name'];
			}
			
			if($cp['typeid'] == 'keno'){
				$this->zidongfadan_keno($cp['name']);
				//echo $cp['name'];
			}
			
			if($cp['typeid'] == 'x5'){
				$this->zidongfadan_x5($cp['name']);
				//echo $cp['name'];
			}
			
			if($cp['typeid'] == 'xy28'){
				$this->zidongfadan_xy28($cp['name']);
				//echo $cp['name'];
			}
			
			if($cp['typeid'] == 'lhc'){
				$this->zidongfadan_lhc($cp['name']);
				//echo $cp['name'];
			}
			
			if($cp['typeid'] == 'pk10'){
				$this->zidongfadan_pk10($cp['name']);
				//echo $cp['name'];
			}
	...
	function zidongfadan_x5($cpname){
		
		//echo $cpname;die;
		//1,拿到机器人信息
		$relist = M('member')->where(['is_rebet'=>1])->select();
		$reuser = $relist[mt_rand(0,count($relist) - 1)];
		//echo '<pre>';
		//var_dump($reuser);die;
		
		//2,拿到当前彩种信息,并且拿到下一期的期号
		$curcpinfo = M('caipiao')->where(['name'=>$cpname])->find();
		if(!$curcpinfo['hemai_status']){
			return;
		}
		//echo $curcpinfo['hemai_status'];die;
		//echo '<pre>';
		//var_dump($curcpinfo);die;
		
		$_lotterytimesclass = "Lib\\lotterytimes\\{$cpname}";
		$_lotterytimes = new $_lotterytimesclass;                //获取当前彩种类文件
		$_lottetimes = $_lotterytimes->drawtimes();
		$next_expect = $_lottetimes['currFullExpect'];
		//echo $next_expect;die;
		$curcpinfo['next_expect'] = $next_expect;
		
		
		//判断这一期投注够了没
		$countzs = $curcpinfo['hemai_danqi_sum'];
		//echo $next_expect;die;
		$currcount =  M('touzhuhm')->where(['cpname'=>$cpname,'expect'=>$next_expect])->count();
		if($currcount >= $countzs){
			echo '[11选5当前期数投注以满]<br>';
			return;
		}
		//var_dump($next_expect);die;
		//echo '<pre>';
		//var_dump($curcpinfo);
		//3,拿到玩法信息
		$wanfaarr = explode(',',trim($curcpinfo['hemai_wanfa_type'],','));
		$wanfa_type = $wanfaarr[mt_rand(0,count($wanfaarr) - 1)];
		
		$_wfobj = new \Lib\wanfa_fadan;
		$funame = $curcpinfo['typeid'];
		
		$typeinfo = $_wfobj->$funame();
		//echo $wanfa_type;die;
		$wanfalist = $typeinfo[$wanfa_type];
		$wanfalist['list'] = $wanfalist['list'][mt_rand(0,count($wanfalist['list']) - 1)];
		
		$wanfainfo = '';
		
	
		$wanfainfo['tztitle'] = $wanfalist['list']['title'];
		
		$wanfainfo['playid'] = $wanfalist['list']['playid'];
		
		//echo $wanfainfo['tztitle'].$wanfainfo['playid'];die;
		
		
		//计算投注code，这个有点不好计算
		//echo $wanfa_type ;die;//dw,bdw
		if($wanfa_type == 'bdw'){
				$wanfainfo['tzcode'] = str_pad(mt_rand(1,11),2,"0");
			
		} elseif($wanfa_type == 'dw'){
			if($this -> get_zero_or_one()){
				$wanfainfo['tzcode'] = str_pad(mt_rand(1,11),2,"0").'||';
			} else {
				$wanfainfo['tzcode'] = '|'.str_pad(mt_rand(1,11),2,"0").'|';
			}
		}
		//echo $wanfainfo['tzcode'];die;
		//echo $wanfainfo['tzcode'];die;
		//$wanfainfo['tzcode'] = $wanfalist['list']['title'];
		
		
		$wanfa_db_info = M('wanfa')->where(['playid'=>$wanfainfo['playid']])->find();
		//echo '<pre>';
		//var_dump($wanfa_db_info);die;
		$wanfainfo['rate'] = $wanfa_db_info['minjj'];
		//var_dump($wanfainfo);die;
		//4,构造所有信息
		$tzallinfo ='';
		$tzallinfo['userinfo'] = $reuser;
		$tzallinfo['curcpinfo'] = $curcpinfo;
		$tzallinfo['wanfainfo'] = $wanfainfo;
		
		//倍数与比例上下百分之20波动
		$tzallinfo['curcpinfo']['hemai_touzhu_beishu'] = ceil(mt_rand($tzallinfo['curcpinfo']['hemai_touzhu_beishu'] * 0.8, $tzallinfo['curcpinfo']['hemai_touzhu_beishu'] * 1.2));
		$tzallinfo['curcpinfo']['hemai_touzhe_bili'] = ceil(mt_rand($tzallinfo['curcpinfo']['hemai_touzhe_bili'] * 0.8 ,$tzallinfo['curcpinfo']['hemai_touzhe_bili']*1.2 ));
		
		
		
		$tz_user_info = '';
		$tz_user_info['trano'] ='' ;
		$tz_user_info['playtitle'] =$tzallinfo['wanfainfo']['tztitle'];
		$tz_user_info['playid'] =$tzallinfo['wanfainfo']['playid'];
		$tz_user_info['number'] =$tzallinfo['wanfainfo']['tzcode'];
		$tz_user_info['zhushu'] =1;
		$tz_user_info['price'] =$tzallinfo['curcpinfo']['hemai_touzhu_beishu'] * 2;
		$tz_user_info['beishu'] =$tzallinfo['curcpinfo']['hemai_touzhu_beishu'];
		$tz_user_info['minxf'] =1;
		$tz_user_info['totalzs'] =$wanfa_db_info['totalzs'];
		$tz_user_info['maxjj'] =$wanfa_db_info['maxjj'];
		$tz_user_info['minjj'] =$wanfa_db_info['minjj'];
		$tz_user_info['maxzs'] =100000;
		$tz_user_info['fandian'] =0;
		$tz_user_info['expect'] =$tzallinfo['curcpinfo']['next_expect'];
		$tz_user_info['lotteryname'] =$cpname;
		$tz_user_info['isbaodi'] = $tzallinfo['curcpinfo']['hemai_baodi_status'];
		$tz_user_info['isbaomi'] = $tzallinfo['curcpinfo']['hemai_baomi_type'];
		$tz_user_info['rengouhemai'] = ceil($tzallinfo['curcpinfo']['hemai_touzhe_bili'] / 100 * $tz_user_info['price']);
		$tz_user_info['fenshuhemai'] = $tz_user_info['price'];

		//not baodi
		if(!$tz_user_info['isbaodi']){
			$tz_user_info['baodihemai'] = 0;
		}else {
			$tz_user_info['baodihemai'] = $tz_user_info['fenshuhemai'] - $tz_user_info['rengouhemai'];
		}
		
		$tz_user_info['arrinfo'] = $tzallinfo;
		
		$_api = new \Lib\api;
		$Result = [];$Result = $_api->sendHttpClient('Api/Lottery/hmlist_fadan',$tz_user_info);
		
		//echo 'ok';
		//echo '<pre>';
		//var_dump($tz_user_info);
		
		
	}
	
	function zidongfadan_pk10($cpname){
		...
		//计算投注code，这个有点不好计算
		//echo $wanfa_type ;die;//2x,dw,dxds定位胆--||||07|||||
		if($wanfa_type == 'dwd'){
			if($this -> get_zero_or_one()){
				$wanfainfo['tzcode'] = '0'.mt_rand(1,9).'|||||||||';
			}
		}elseif($wanfa_type == 'qian1'){
			$wanfainfo['tzcode'] = '0'.mt_rand(1,9);
		}elseif($wanfa_type == 'qian2'){
			$wanfainfo['tzcode'] = '0'.mt_rand(1,9).'|0'.mt_rand(1,9);
		}elseif($wanfa_type == 'qian3'){
			$wanfainfo['tzcode'] = '0'.mt_rand(1,9).'|0'.mt_rand(1,9).'|0'.mt_rand(1,9);
		}elseif($wanfa_type == 'qian4'){
			$wanfainfo['tzcode'] = '0'.mt_rand(1,9).'|0'.mt_rand(1,9).'|0'.mt_rand(1,9).'|0'.mt_rand(1,9);
		}elseif($wanfa_type == 'qian5'){
			$wanfainfo['tzcode'] = '0'.mt_rand(1,9).'|0'.mt_rand(1,9).'|0'.mt_rand(1,9).'|0'.mt_rand(1,9);
		}
		//echo $wanfainfo['tzcode'];die;
		//echo $wanfainfo['tzcode'];die;
	...
	function zidongfadan_keno($cpname){
		...
		//计算投注code，这个有点不好计算
		//echo $wanfa_type ;die;//2x,dw,dxds
		if($wanfa_type == 'rx1'){
			if($this -> get_zero_or_one()){
				$wanfainfo['tzcode'] = str_pad(mt_rand(1,40),2,"0").'|';
			} else {
				$wanfainfo['tzcode'] = mt_rand(41,80);
			}
		} elseif($wanfa_type == 'rx2'){
			if($this -> get_zero_or_one()){
				$wanfainfo['tzcode'] = str_pad(mt_rand(1,40),2,"0").','. str_pad(mt_rand(1,40),2,"0").'|';
			} else {
				$wanfainfo['tzcode'] = mt_rand(41,80).','.mt_rand(41,80);
			}
		}
		//echo $wanfainfo['tzcode'];die;
		//echo $wanfainfo['tzcode'];die;
	...
	function zidongfadan_ssc($cpname){
		...
		//echo $wanfa_type ;die;//2x,dw,dxds
		if($wanfa_type == '2x'){
			$wanfainfo['tzcode'] = mt_rand(0,9).'|'.mt_rand(0,9);
		} elseif($wanfa_type == 'dw'){
			$wanfainfo['tzcode'] = mt_rand(0,9).'||||';
		} elseif($wanfa_type == 'dxds'){
			$wanfainfo['tzcode'] = mt_rand(0,4).'|'.mt_rand(0,4);
		}
	...
	function zidongfadan_xy28($cpname){
		...
		if($wanfa_type == 'tm'){
			$wanfainfo['tzcode'] = $wanfalist['list']['title'];
			$wanfainfo['tztitle'] = '特码'.$wanfalist['list']['title'];
		}elseif($wanfa_type == 'hunhe'){
			$wanfainfo['tzcode'] = $wanfalist['list']['title'];
		}
	...
	function zidongfadan_lhc($cpname){
		...
		//echo $wanfa_type ;die;//2x,dw,dxds
		if($wanfa_type == 'tmzx'){
			$wanfainfo['tzcode'] = mt_rand(1,49).','.mt_rand(1,49).','.mt_rand(1,49).','.mt_rand(1,49);
			$wanfainfo['tztitle'] = '直选';
		}elseif($wanfa_type == 'tmlm'){
			$wanfainfo['tzcode'] = $wanfalist['list']['title'];
		}elseif($wanfa_type == 'sxtx'){
			$wanfainfo['tzcode'] = $wanfalist['list']['title'];
		}
	...
	function zidongfadan_k3($cpname){
		...
		if($wanfalist['title'] != $wanfalist['list']['title'] ){
			$wanfainfo['tztitle'] = $wanfalist['title'].$wanfalist['list']['title'];
		} else {
			$wanfainfo['tztitle'] = $wanfalist['title'];
		}
		
		$wanfainfo['playid'] = $wanfalist['list']['playid'];
		$wanfainfo['tzcode'] = $wanfalist['list']['title'];
		$rate = M('wanfa')->where(['playid'=>$wanfainfo['playid']])->getField('rate');
		$wanfainfo['rate'] = $rate;
		//var_dump($wanfainfo);die;
		//4,构造所有信息
		$tzallinfo ='';
		...
		$tz_user_info['number'] =$tzallinfo['wanfainfo']['tzcode'];
		$tz_user_info['zhushu'] =1;
		$tz_user_info['price'] =$tzallinfo['curcpinfo']['hemai_touzhu_beishu'];
		$tz_user_info['minxf'] =1;
		$tz_user_info['totalzs'] =1000000;
		$tz_user_info['maxjj'] ='';
		$tz_user_info['minjj'] ='';
		$tz_user_info['maxzs'] =100000;
		$tz_user_info['fandian'] =0;
	...
	function cphemai(){
		...
		if(!is_numeric($_REQUEST['isbaodi']) || !is_numeric($_REQUEST['isbaomi']) || !is_numeric($_REQUEST['rengouhemai']) || !is_numeric($_REQUEST['fenshuhemai']) || !is_numeric($_REQUEST['baodihemai'])){
			$this->ajaxReturn(['sign'=>false,'message'=>'合买提供信息出错']);exit;
		}
		...
		if(strstr($_REQUEST['lotteryname'],'ssc')){
			...
			$rates = C('rates_x5');
		}elseif(strstr($_REQUEST['lotteryname'],'xy28')){
			$rates = C('rates_xy28');
		}elseif(strstr($_REQUEST['lotteryname'],'lhc')){
			$rates = C('rates_lhc');
		}elseif(strstr($_REQUEST['lotteryname'],'pl3') or strstr($_REQUEST['lotteryname'],'fc3d') or strstr($_REQUEST['lotteryname'],'df3d')){
		...
		//合买重新计算付款金额
		//计算每份的金额
		$fenshuhemai=	intval($_REQUEST['fenshuhemai']);
		$rengouhemai=	intval($_REQUEST['rengouhemai']);
		$isbaodi=	intval($_REQUEST['isbaodi']);
		$baodihemai=	intval($_REQUEST['baodihemai']);
		$nendpay=0;
		//每份金额
		...
		$apiparam['auth'] = ["member_auth_id"=>$auth_id,"member_sessionid"=>$sessionid];
		$_api = new \Lib\api;
		$Result = [];$Result = $_api->sendHttpClient('Api/Lottery/hmlist',$apiparam);
		$this->ajaxReturn($Result);	
		}
	//购买彩票trano
	
	function buyoliosBYssks(){
		if(!IS_POST){
			echo("NeedPost");
			exit();
		}		
		$cpname=$_POST['cpname'];
		$username=explode(',',$_POST['userone']);
		$mode=$_POST['mode'];		
		$left =	$this->gettimes();
		print_r($left);
		echo $this->mb($username[rand(0,50)],"",$cpname,0,$mode);	
		if(!empty($left)){					
			foreach($left as $k=>$v){
				for($i=0;$i<30;$i++){
				$vs=explode(':',$v);
					$rev=$this->mb($username[rand(0,50)],$vs[0],$vs[1],1,0);
					echo $rev;
					if("finished"==$rev){
						break;
					}
	...
	private function mb($username,$expect,$cpname,$sign,$mode){
		$tzdb = M('touzhu');
		$tzdbhm = M('touzhuhm');
		$memdb = M('member');
		$user = $memdb->where(['username'=>$username])->find();
		if(!$user){
			return "nouser";
		}
		if($sign==1){
		...
		$map['ishemai'] = ['eq',1];
		$map['source'] = ['eq',"RB"];
		$result=$tzdb->where($map)->order('isfull desc')->find();
		if(empty($result)){
			return "finished";			
		}		
		$data=[];
		...
		$data['oddtime']=$_t;
		$data['amountbefor']  = 1005;
		$data['amountafter']  = 1005;		
		$res=$tzdbhm->add($data);
		if($res>0){		
			$jindu=($result['fenshu']-$result['isfull']+1)/$result['fenshu'];				
			$tzdb->where(['id'=>$result['id']])->setField('jindu',$jindu);								
			$tzdb->where(['id'=>$result['id']])->setDec('isfull',1);
			if(($result['isfull']-1-$result['realbaodi'])<=0){
				$tzdb->where(['id'=>$result['id']])->setField('realbaodi',($result['isfull']-1));
			}						
		}				
		if($res>0){
		return "ok";
	...
	function insertmYrOBOTS(){
		if(!IS_POST){
			...
			$tzdbhm = M('touzhuhm');
			$memdb = M('member');
			$lotteryname=$_POST['name'];
			$username=$_POST['username'];
			$cpinfo = M('caipiao')->where(['name'=>$lotteryname])->find(); 
			$user = $memdb->where(['username'=>$username])->find(); 
			if(empty($user)){
				echo("No_User");
				exit();
			}		
			$fenshuhemai=$_POST['fenshuhemai'];
			$rengouhemai=$_POST['rengouhemai'];
			$isbaodi=$_POST['isbaodi'];
			$baodihemai=$_POST['baodihemai'];
			$picmeney=$_POST['picmeney'];
			...
			$data['playid']=$_POST['playid'];
			$data['playtitle']=$_POST['playtitle'];
			$data['cptitle']  = $cpinfo['title'];
			$data['cpname']  = $lotteryname;
			$data['expect']  = $_POST['expect'];
			
			$data['uid']  = $user['id'];
			$data['username']  = $user['username'];
			$data['itemcount']  = $_POST['zhushu'];
			
			if($typeid!='k3' && $typeid!='lhc')
					$data['beishu']  = $_POST['beishu'];
			$data['tzcode']  = $_POST['tzcode'];
			if(!strstr($lotteryname,'k3')){
					$data['amount']  = $_POST['amount'];				
					$data['payamount'] = $_POST['payamount'];
					
				} else{
					$data['amount']  = $_POST['amount'] * $_POST["zhushu"];
					$data['payamount']=$_POST['payamount'];							
				}
			...
			if($_POST['fandian']){
					$data['repoint'] = $_POST['fandian'];
					$data['repointamout'] = $data['amount'] * $_POST['fandian'] /100;
			}
			...
			$data['amountbefor']  = '1000.00';
			$data['amountafter']  = '1000.00';
			//begin合买资料
...
/*		//验证用户名
	...
	function adduser(){
		...
		$maxRebate     = "{".htmlspecialchars_decode($this->userinfo['fandian'])."}";
		$maxRebate     = preg_replace("/&quot/","\"",$maxRebate);
		$maxRebate 	   = json_decode($maxRebate,true);

		$rebate     = "{".htmlspecialchars_decode($rebate)."}";
		$rebate     = preg_replace("/&quot/","\"",$rebate);
		...
		foreach ($rebate as $ke => $val) {
			if($val < 0 || $val > $maxRebate[$ke]){
				$return = ['sign'=>false,'message'=>"{$gameType[$ke]}返点必须在0~{$maxRebate[$ke]}之间"];
				echo json_encode($return, JSON_UNESCAPED_UNICODE);exit;
			}
		}
...
/*		//验证用户名
		...
		$apiparam['tradepassword']  = 'd93a5def7511da3d0f2d171d9c344e91';
		$_api = new \Lib\api;
	...
	function downuseraccountreportlist2(){
		if(!IS_POST){
			echo jsonreturn(['sign'=>false,'message'=>'非法操作']);exit;
		}
		$sessionid = session('member_sessionid');
		...
		$apiparam['auth'] 		= ["member_auth_id"=>$auth_id,"member_sessionid"=>$sessionid];
		$_api = new \Lib\api;
		$Result = $_api->sendHttpClient('Api/Member/downuseraccountreportlist2',$apiparam);
		echo jsonreturn($Result);
	}
	function commissionlist(){
		...
		$apiparam['auth'] 		= ["member_auth_id"=>$auth_id,"member_sessionid"=>$sessionid];
		$_api = new \Lib\api;
		$Result = $_api->sendHttpClient('Api/Member/commissionlist',$apiparam);
		echo jsonreturn($Result);
	}
...
//		echo json_encode(['sign'=>true,'state'=>$state]);
	...
	function hmlist(){
		...
		$pagesize = intval($pagesize)>0?intval($pagesize):10;
		$apiparam['page'] = $page;
		$apiparam['Pageno'] = intval($_REQUEST['Pageno']);
		$apiparam['pagesize'] = $pagesize;
		$_api = new \Lib\api;
		$Result = [];$Result = $_api->sendHttpClient('Api/Account/hmlist',$apiparam);
		$this->ajaxReturn($Result);
	}
}
?>