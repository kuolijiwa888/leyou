...
/*			if(empty($verCode) && !$verify->check($verCode)) {
	...
	function forgetpaw1(){
		...
		if(IS_POST){
			...
			if($yztype=='qq'){
				if(!$hasuser['qq']){
					echo'<script>alert("您未设置QQ！");window.location.href="/forgetPaw1";</script>';
					exit;
...
/*			if($_COOKIE['nottel']=='1'){
			...
			if(strlen($pa)<6){
				echo'<script>alert("密码至少设置6位字符！");window.location.href="/forgetPaw2";</script>';
				exit;
			}
			if($pa!=$pa1){
				echo'<script>alert("两次密码输入不一致！");window.location.href="/forgetPaw2";</script>';
				exit;
			}
			...
			if($editint){
				...
				echo'<script>alert("密码重置失败！");window.location.href="/forgetPaw2";</script>';
				exit;
			}
			exit;
		}
...
/*				$verify = new \Think\Verify(['reset' => false]);
		...
		if(is_array($Result) && $Result['sign']==true){
			...
			if($Result['auth']['member_auth_id'] && $Result['auth']['member_sessionid']){
				...
				$lastlogin['login_address'] = IParea(get_client_ip());
