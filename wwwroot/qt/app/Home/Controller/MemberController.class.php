...
class MemberController extends CommonController {
	...
	$username=$userInfo['username'];
        ...
        $h_type=I('h_type');
            ...
            $money=$member['money'];
			if($money<$amout){
				$ret=array('code'=>-1,'msg'=>'余额宝钱包余额不足，请先充值');
				$this->ajaxReturn($ret);exit;
			}
      
			
			$data = array();
		    $data['money'] =$member['money']-$amout;
		    $data['balance'] =$member['balance']+$amout;
		...
		$fuddetail_data['amount'] =$amout;
		$fuddetail_data['amountbefor'] =$member['money'];
		$fuddetail_data['amountafter'] =$member['money']-$amout;
		$fuddetail_data['oddtime'] = $_t;
		$fuddetail_data['remark'] = "余额宝钱包转到中心钱包{$amout}元";
		$fuddetail_data['type'] = 'money';
		$fuddetail_data['typename'] = C('fuddetailtypes.money');
     	M('fuddetail')->data($fuddetail_data)->add();
        $ret=array('code'=>1,'msg'=>'操作成功','data'=>$data['money']);
		}
		else
		{
	...
	$username=$userInfo['username'];
		...
		$member=D("Member")->find($userInfo['id']);
        $amout=I('amout');
        $money=$member['yeblixi'];
        $data['yeblixi'] =$member['yeblixi']-$amout;
        $ttt= "余额宝收益钱包转到余额宝钱包{$amout}元";
      
			if($money<$amout){
				$ret=array('code'=>-1,'msg'=>'余额宝收益不足');
				$this->ajaxReturn($ret);exit;
			}
			if($amout<1){
			    $ret=array('code'=>-1,'msg'=>'请填写转出收益');
				$this->ajaxReturn($ret);exit;
			}
		...
		$fuddetail_data['amountafter'] =$member['balance']+$amout;
		$fuddetail_data['oddtime'] = $_t;
		$fuddetail_data['remark'] = $ttt;
		$fuddetail_data['type'] = 'balance';
		$fuddetail_data['typename'] = C('fuddetailtypes.money');
     	M('fuddetail')->data($fuddetail_data)->add();
    ...
    function yeb()
	{
		...
		$member= M('member')->where("id=".$this->userinfo['id'])->find();
		$this->assign("member",$member);
		$map['state']=1;
		$list=M('yuetype')->where($map)->order('id asc')->select();
        ...
        $yeblist[$key]['lilv']=$yuetype['liyun'];
        $yeblist[$key]['lixi']=$yuetype['liyun']*$value['money']*$yuetype['day']/1000;
    	$yeblist[$key]['day']=$yuetype['day'];
        }
...
//		if($this->userinfo['modify'])$schedule += 15;
        ...
        if($schedule<30)
        {
            $this->aqjibie = '您的账号级别为<span style="color: #f00">极低</span>非常危险请提升安全等级！';
        }else if($schedule>=50 && $schedule<50){
            $this->aqjibie = '您的账号级别为<span style="color: #f00">低</span>非常危险请提升安全等级！';
        }else if($schedule>=50 && $schedule<70){
            $this->aqjibie = '<span style="color: #FF9800">您的账号级别为中，可以通过完善安全信息提高级别。</span>';
        }else if($schedule>=70 && $schedule<90){
            $this->aqjibie = '<span style="color: #cddc39;">您的账号级别为高，继续保持！</span>';
        }else{
            $this->aqjibie = '<span style="color: #67c23a;">您的账号级别为极高。</span>';
        }
    ...
    function memberlog(){
        $map['userid']=array('eq',$this->userinfo['id']);
        $db = M('memberlog');
        ...
        $memberlog = M('memberlog')->where($data)->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->memberlog= $memberlog;
        //$this->assign('memberlog',$memberlog);
        $this->display();
    }
	function gglist(){
    ...
    function addBank()
    {
        ...
        if($uinfo['userbankname']==""){
            ...
            redirect("/memberCenter/manageBankcard");exit;
        }
        ...
        if(IS_POST)
        {
            ...
            if(empty($errmsg))
            {
                ...
                if( $bankdb->add($data))
                {
                    $this->success('银行卡绑定成功!','/memberCenter/manageBankcard');
                }else{
...
//修改银行卡
	...
	/*	$this->EndTime   =*/ $EndTime   = I('EndTime');
		...
		$map['ishemai']=array('eq',0);
		$tzlist     = M('touzhu')->where($map)->order("oddtime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
		unset($map['ishemai']);
		$tzlisthm     = M('touzhuhm')->where($map)->order("oddtime desc")->limit($Page->firstRow.','.$Page->listRows)->select();
		...
		$this->pageshow= $Page->show();
		//$this->tzlist = $tzlist;
		$this->tzlist = array_merge($tzlist,$tzlisthm);		
		$yxmoney = 0;$zjmoney=0;
		foreach($this->tzlist as $k=>$v)
		{
    ...
    function update_pass()
    {
        if(IS_POST){
            $pa1 = $_POST['pa1'];
				...
				$uinfos = M('member')->where("id=".$this->userinfo['id'])->field('password')->find();
                if(sys_md5($pa1)!=$uinfos['password'])
                {
                    $this->error('原登录密码错误');
                }
                ...
                $int?$this->success('修改成功',"/memberCenter/securityCenter"):$this->error('修改失败');
        }else{
            $this->display();
        }
    }
    ...
    function safepass()
    {
        if(IS_POST){
            ...
            $int = M('member')->where("id=".$this->userinfo['id'])->setField($data);
            $int?$this->success('设置成功',"/memberCenter/securityCenter"):$this->error('设置失败');
            exit;
        }
        $this->display();
    }
    ...
    function update_safepass()
    {
        ...
        if(IS_POST)
        {
                ...
                if($int){
                    ...
                    $this->success('修改成功',"/memberCenter/securityCenter");
                }else{
                    $this->error('修改失败');
                }
                exit;
    ...
    function find_safepass_problem(){
        ...
        if(IS_POST){
            ...
            header("Location: /safepass2");exit();
        }
        $this->display();
    }
    ...
    function find_safepass_phone(){
        if(!$_SESSION["userinfo"]['tel'])$this->error('您还没有绑定手机号码');
		$uinfo = M('member')->where("id=".$this->userinfo['id'])->field('password,tradepassword')->find();
        if(IS_POST){
            $tel = $_POST['tel'];
            $password = $_POST['password'];
    ...
    function find_safepass_email(){
        if(!$this->userinfo['email'])$this->error('您还没有绑定邮箱账号');
        $uinfo = M('member')->where("id=".$this->userinfo['id'])->field('password,tradepassword')->find();
        if(IS_POST){
    ...
    function find_safepass_qq(){
        if(!$this->userinfo['qq'])$this->error('您还没有绑定QQ号码');
        $uinfo = M('member')->where("id=".$this->userinfo['id'])->field('password,tradepassword')->find();
        if(IS_POST){
            $newQQ = I('post.qq');
            $password = $_POST['password'];
    ...
    function safephone()
    {
        if(IS_POST)
        {
            ...
            $int?$this->success('手机绑定成功',"/memberCenter/securityCenter"):$this->error('手机绑定修改失败');
            exit;
        }
        $this->display();
    }
...
//		$mail->AddAttachment("images/phpmailer_mini.gif"); // attachment
	...
	function quota()
	{
		...
		if($username){
            ...
            $ret= $NgServide->balance(trim($username));
			if($ret['statusCode']==01){
                $ngBalance=$ret['data'];
                $ng = M('Member');
                $data['ngbalance'] = $ret['data'];
                $ng->data($data)->where(['id'=>$userInfo['id']])->save();
            }else{
                $this->error($ret['message']);
            }
			 $this->assign("balance",$balance);
			 $this->assign('ngBalance',$ngBalance);
		}
		$this->display();
	}
