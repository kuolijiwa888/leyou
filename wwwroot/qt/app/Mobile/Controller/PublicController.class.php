...
class PublicController extends CommonController {
	...
	function register(){
		if($_SESSION['userinfo']){
			$this->error('你已经登录,请先退出再注册','/memberCenter/securityCenter');
			exit();
		}
		//无邀请码
		$defaulttjcode = M('setting')->where("name='defaulttjcode'")->getField('value');
...
//				$this->success('恭喜你!注册成功',U('Member/index'));
	...
	function forgetPaw(){
		if(IS_POST){
			...
			if(!$userName) {
				//echo'<script>alert("请填写用户名！");window.location.href="/forgetPaw";;</script>';
				$return['sign']    = false;
			...
			if(!$hasuser) {
				//echo'<script>alert("用户未找到！");window.location.href="/forgetPaw";</script>';
				$return['sign']    = false;
			...
			//redirect('/userCenter/verifySafePwd');
			$return['sign']    = true;
		    $return['message'] = '用户名正确';
			$this->ajaxReturn($return);
			exit;
		}
		$this->display();
	...
	function forgetPaw1(){
		...
		if(IS_POST){
			...
			if($yztype=='tel'){
				if(!$hasuser['tel']){
					//echo'<script>alert("您未绑定手机号码！");window.location.href="/forgetPaw1";</script>';
					$return['sign']    = false;
			        $return['message'] = '您未绑定手机号码！';
			        $this->ajaxReturn($return);
					exit;
				}elseif($yztext!=$hasuser['tel']){
					//echo'<script>alert("手机号码验证错误！");window.location.href="/forgetPaw1";</script>';
					$return['sign']    = false;
			        $return['message'] = '手机号码验证错误！';
			        $this->ajaxReturn($return);
					exit;
				}else{
					//echo'<script>alert("手机验证成功！");window.location.href="/forgetPaw2";</script>';
					$return['sign']    = true;
			        $return['message'] = '手机验证成功！';
			        $this->ajaxReturn($return);
			        exit;
				}
			}elseif($yztype=='email'){
				if(!$hasuser['email']){
					//echo'<script>alert("您未绑定邮箱！");window.location.href="/forgetPaw1";</script>';
					$return['sign']    = false;
			        $return['message'] = '您未绑定邮箱！';
			        $this->ajaxReturn($return);
					exit;
				}elseif($yztext!=$hasuser['email']){
					//echo'<script>alert("绑定邮箱验证错误！");window.location.href="/forgetPaw1";</script>';
					$return['sign']    = false;
			        $return['message'] = '绑定邮箱验证错误！';
			        $this->ajaxReturn($return);
					exit;
				}else{
					//echo'<script>alert("邮箱验证成功！");window.location.href="/forgetPaw2";</script>';
					$return['sign']    = true;
				...
				if(!$hasuser['qq']){
					//echo'<script>alert("您未绑定QQ！");window.location.href="/forgetPaw1";</script>';
					$return['sign']    = false;
			        $return['message'] = '您未绑定QQ！';
			        $this->ajaxReturn($return);
					exit;
				}elseif($yztext!=$hasuser['qq']){
					//echo'<script>alert("QQ验证错误！");window.location.href="/forgetPaw1";</script>';
					$return['sign']    = false;
			        $return['message'] = 'QQ验证错误！';
			        $this->ajaxReturn($return);
					exit;
				}else{
					//echo'<script>alert("qq验证成功！");window.location.href="/forgetPaw2";</script>';
					$return['sign']    = true;
			        $return['message'] = 'qq验证成功！';
			        $this->ajaxReturn($return);
			        exit;
				}
			}
...
//			echo jsonreturn($Result);exit;
		...
		if(is_array($Result) && $Result['sign']==true){
			...
			if($Result['auth']['member_auth_id'] && $Result['auth']['member_sessionid']){
				...
				$lastlogin['login_address'] = 'null';//json_decode(getIpAddress($lastlogin['lastip']))->region.",".json_decode(getIpAddress($lastlogin['lastip']))->city;
