...
//k3
	function k3(){
		...
		$_k3lists = M('Caipiao')->field('*')->where("isopen='1' AND iswh='0' AND typeid='k3'")->order('listorder ASC')->select();
			foreach($_k3lists as $k=>$v){
			// if($v['isopen']==0)unset($_ssclist[$k]);
			$_k3list[$v['name']]=$_k3lists[$k];
		}
		...
		foreach($rates as $k=>$v){
			$peilv[$v['playid']] = $Result['data'][$v['playid']]['maxjj'];
		}
...
//SSC
	function ssc(){
		...
		// $_ssclist = C('cplist.ssc');
		 ...
		 $_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='ssc'")->order('listorder ASC')->select();
		//
		foreach($_ssclists as $k=>$v){
			// if($v['isopen']==0)unset($_ssclist[$k]);
			$_ssclist[$v['name']]=$_ssclists[$k];
		}
		 //print_r($_ssclist);exit();
	...
	function pk10(){
		...
		$_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='pk10'")->order('listorder ASC')->select();
		//
		foreach($_ssclists as $k=>$v){
			// if($v['isopen']==0)unset($_ssclist[$k]);
			$_ssclist[$v['name']]=$_ssclists[$k];
	...
	function keno(){
		...
		$_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='keno'")->order('listorder ASC')->select();
		//
		foreach($_ssclists as $k=>$v){
			// if($v['isopen']==0)unset($_ssclist[$k]);
			$_ssclist[$v['name']]=$_ssclists[$k];
	...
	function x5(){
		...
		$_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='x5'")->order('listorder ASC')->select();
		// print_r($_ssclists);exit;
		foreach($_ssclists as $k=>$v){
			// if($v['isopen']==0)unset($_ssclist[$k]);
			$_ssclist[$v['name']]=$_ssclists[$k];
		}
	...
	$_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='dpc'")->order('listorder ASC')->select();
		//
		foreach($_ssclists as $k=>$v){
			// if($v['isopen']==0)unset($_ssclist[$k]);
			$_ssclist[$v['name']]=$_ssclists[$k];
	...
	function lhc(){
		...
		// $_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='lhc'")->order('listorder ASC')->select();
		//
		foreach($_ssclists as $k=>$v){
			 if($v['isopen']==0)unset($_ssclist[$k]);
			// $_ssclist[$v['name']]=$_ssclists[$k];
	...
	function xy28(){
		...
		$lotteryname = I('code');
		$this->assign('lotteryname',$lotteryname);
		// print_r($lotteryname);exit();
	     // $_ssclists = M('Caipiao')->where("isopen='1' AND iswh='0' AND typeid='xy28'")->order('listorder ASC')->select();
		$_lhclist = C('cplist.xy28');
		foreach($_ssclists as $k=>$v){
			 if($v['isopen']==0)unset($_ssclist[$k]);
			// $_ssclist[$v['name']]=$_ssclists[$k];
		}
