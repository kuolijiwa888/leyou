...
class MemberController extends CommonController {
    ...
    { $map['uid']=array('eq',$this->userinfo['id']);
        $db = M('NoticeSee');
        $count      = $db->join('caipiao_notice ON caipiao_notice_see.nid = caipiao_notice.id')->where('caipiao_notice_see.uid='.$this->userinfo['id'])->where('caipiao_notice_see.is_see=0')->count();
        $schedule = 30;
...
//		if($this->userinfo['modify'])$schedule += 15;
    ...
    function xiaoxi()
    {
        ...
        $map['uid']=array('eq',$this->userinfo['id']);
        $db = M('NoticeSee');
        $count      = $db->join('caipiao_notice ON caipiao_notice_see.nid = caipiao_notice.id')->where('caipiao_notice_see.uid='.$this->userinfo['id'])
->count();
                         
        $Page       = new \Think\Page($count,10);
        startPage($Page);
        $mxlist     = $db->join('caipiao_notice ON caipiao_notice_see.nid = caipiao_notice.id')->where('caipiao_notice_see.uid='.$this->userinfo['id'])->order("caipiao_notice_see.add_time desc")->limit($Page->firstRow.','.$Page->listRows)->select();
            $this->pageshow= $Page->moblie_show();
        $this->mxlist = $mxlist;
        $this->display();
    }
    function xiaoinfo()
    {
        ...
        $ggshow = M('NoticeSee')->join('caipiao_notice ON caipiao_notice_see.nid = caipiao_notice.id')->where('caipiao_notice_see.uid='.$this->userinfo['id'])->where('caipiao_notice.id='.$aid)->find();
         $ao=[];
         $ao['is_see']=1;
         $map['uid']=array('eq',$this->userinfo['id']);
         $pp['nid']=array('eq',$aid);
         $int = M('NoticeSee')->where($map)->where($pp)->data($ao)->save();
        $this->assign('ggshow',$ggshow);
        $this->display();
    ...
    function betrecord()
    {
        ...
        /*	$this->a_item    =*/ $a_item    = I('itemselect',1);
        ...
        $this->pageshow= $Page->moblie_show('itemselect/'.$a_item);
        $this->tzlist = $tzlist;
        $yxmoney = 0;$zjmoney=0;
...
//			redirect(U("Member/addbank"));exit;
        ...
        if(IS_POST)
        {
            ...
            if(empty($errmsg))
            {
                ...
                if( $bankdb->add($data))
                {
                    $this->success('银行卡绑定成功!',"/memberCenter/securityCenter");
                }else{
