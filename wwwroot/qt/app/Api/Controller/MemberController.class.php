...
class MemberController extends CommonController {
  ...
  function adduser($apiparam=array()){
    ...
    $data['fandian']  = json_encode($apiparam['rebate']);
    $data['fandian'] = preg_replace('/^\{/', '', $data['fandian']);
    $data['fandian'] = preg_replace('/\}$/', '', $data['fandian']);
    $data['isnb']     = 0;
...
/*  function getids(){
  ...
  function downuserbetslist($apiparam=array()){
    ...
    if(!$downlist){
      $apiparam['sign'] = false;
      $apiparam['message'] = '获取成功';
      $apiparam['startime'] = date('Y-m-d H:i:s',$apiparam['startime']);
      $apiparam['endtime'] = date('Y-m-d 23:59:59',$apiparam['endtime']);
      $apiparam['page'] = 0;
      $apiparam['total'] = 0;
      $apiparam['records'] = 0;
      $apiparam['root'] = [];
    ...
    $apiparam['endtime'] = date('Y-m-d 23:59:59',$endtime);
    $apiparam['page'] = $page;
    $apiparam['pagestr'] = $GridPage.','.$pagesize;
    $apiparam['total'] = $totalsize;
    $apiparam['records'] = $records;
  ...
  function downuseraccountreportlist2($apiparam=array()){
    $apiparam = self::_cheacktoken($apiparam);
    if(!$apiparam['sign'])return $apiparam;
    $apiparam = checklogin($apiparam);
    if(!$apiparam['sign'])return $apiparam;
    ...
    $beginToday= mktime(0,0,0,date('m'),date('d'),date('Y'));
    $endToday  = mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
    $sDate  = date("Y-m-d 00:00:00",$apiparam['startime']);//时间戳
    $eDate   = date("Y-m-d 00:00:00",strtotime(date('Y-m-d 00:00:00',$apiparam['endtime'])));//时间戳
    $username = $apiparam['loginname'];
    $pid = $apiparam['pid'];
    ...
    if(empty($pid)){
      // $map1['id'] = ['eq',$userinfo['id']];
      $map1['parentid']    = ['eq',$userinfo['id']];
    }else{
      $map1['parentid']    = ['eq',$pid];
    }
    //$map1['proxy']    = ['eq',1];
    if(!empty($username)){
    ...
    $show       = $Page->show();*/
    $list       = M('member')->where($map1)->order('id desc')->field('id,username,proxy')->select(); //->limit($Page->firstRow.','.$Page->listRows)
    foreach($list as $k=>$v){
      //获取所有下线用户
      ...
      //投注盈亏
      $v['tzyingkui'] = 0;
      $v['tzyingkui'] = $v['zhongjiang']-$v['touzhu'];
      $list[$k] = $v;
      // var_dump($list);exit;
    }
    foreach ($list as $key => &$val) {
      $val['isxj']          = $val['totalcount']>0?1:0;
      $val['dayRechargeMoney']    = $val['zdrecharge']+$val['sdjiarecharge']-$val['sdjianrecharge'];
      $val['dayDrawRechargeMoney']  = empty($val['withdraw'])?0:$val['withdraw'];
      $val['dayConsumptionMoney']   = empty($val['touzhu'])?0:$val['touzhu'];
      $val['dayCommissionMoney']    = empty($val['zhongjiang'])?0:$val['zhongjiang'];
      $val['dayActivitiesMoney']    = 0;
      $val['dayDividendMoney']    = $val['zhongjiang']-$val['touzhu'];
    }
    $list1 = array_slice($list,($page-1)*$pagesize,$pagesize);
    if($list){
      foreach($list as $k=>$v){
    ...
    $apiparam['sign'] = true;
    $apiparam['message'] = '获取成功';
    $apiparam['startime'] = $sDate;
    $apiparam['endtime'] =$eDate;
    $apiparam['root'] = $list;
    $apiparam['totaldata'] = $totaldata;
  ...
  function commissionlist($apiparam=array()){
    $apiparam = self::_cheacktoken($apiparam);
    if(!$apiparam['sign'])return $apiparam;
    $apiparam = checklogin($apiparam);
    if(!$apiparam['sign'])return $apiparam;
    ...
    if(empty($username) && empty($pid)){
      array_unshift($list,array("id"=>$userinfo['id'],"username"=>$userinfo['username'],"proxy"=>$userinfo['proxy']));
    }
    ...
    foreach($list as $k=>$v){
      ...
      $list[$k]['totalcount'] = $totalcount?$totalcount:0;
      //佣金
      $map = [];
      $XUIDS[] = $v['id'];
      $map['uid'] = ['in',$XUIDS];
      if($loginname)$map['username'] = ['eq',$loginname];
      $map['type'] = ['in',['yongjinshenhe']];
      if($sDate)$map['oddtime'][]    = ['gt',strtotime($sDate)];
      if($eDate)$map['oddtime'][]    = ['elt',strtotime($eDate)+86400-1];
      $commission = 0;
      // var_dump($map);exit;
      $commission = M('fuddetail')->where($map)->sum('amount'); //sum('amount')
      $list[$k]['commission'] = $commission?$commission:0;
      
    }
    // var_dump($list);exit;
    foreach ($list as $key => &$val) {
      $val['isindex']     = $val['id']==$userinfo['id']?1:0;
      $val['isxj']      = $val['totalcount']>0?1:0;
      $val['commission']    = $val['commission']?$val['commission']:0;
    }
  ...
  function getdownlist($data,$lv = 1){
    // var_dump($data);
    for ($i = 0; $i < count($data); $i++) {
      $data[$i]['lv'] = $lv;
      $map = [];
      $map['parentid'] = ['eq',$data[$i]['id']];
      $map['isnb'] = ['eq',0];
      $this->tree[count($this->tree)] = $data[$i]['id'];
      $res = M('member')->where($map)->order('id desc')->field('id')->select();
      $this->getdownlist($res, ($lv + 1));
    }
  }
  //注册
...
/*    if($userinfo['proxy']==1){
  ...
  function echarts($apiparam=array()){
    ...
    for($i=0,$j=$days;$i<=$days;$i++,$j--){
      $ascntime[] = $_tt = date("Y-m-d", strtotime($_endDate)-86400*$j);
      $map = [];
      $map['uid'] = ['in',$chiduids];
      $map['state'] = ['eq',1];
      $map['oddtime'][] = ['elt',strtotime($_tt)+86400-1];
      ...
      $czdata[] = $amount?$amount:0;
    }
    $czsubtext = '单位(元)';
    $cztext = '充值金额';
    for($i=0,$j=$days;$i<=$days;$i++,$j--){
      $_tt = date("Y-m-d", strtotime($_endDate)-86400*$j);
      $map = [];
      $map['uid'] = ['in',$chiduids];
      $map['state'] = ['eq',1];
      $map['oddtime'][] = ['elt',strtotime($_tt)+86400-1];
    ...
    $apiparam['startime']    = date('Y-m-d H:i:s',$startime);
    $apiparam['endtime']    = date('Y-m-d H:i:s',$endtime);
    $apiparam['date'] = $ascntime;
    $apiparam['data'] = $ascndata = array('czdata'=>$czdata,'txdata'=>$txdata,'tzdata'=>$tzdata,'fddata'=>$fddata,'xzdata'=>$xzdata);
    $apiparam['subtext'] = $subtext = array('czsubtext'=>$czsubtext,'txsubtext'=>$tzsubtext,'tzsubtext'=>$tzsubtext,'fdsubtext'=>$fdsubtext,'xzsubtext'=>$xzsubtext);
    $apiparam['text'] = $text = array('cztext'=>$cztext,'txtext'=>$txtext,'tztext'=>$tztext,'fdtext'=>$fdtext,'xztext'=>$xztext);
    return $apiparam;
  }
  //收件箱
