	...
	function chedan($apiparam=array()){
		...
		if($_lottetimes['currFullExpect']!=$info['expect']){
			...
			if($_int){
				...
				// 	}
				// }
				$pidinfo = M('dailifandian')->where(['xiayiid'=>$userinfo['id'],'trano'=>$trano])->find();

				if ($pidinfo['uid']>0) {
                 
				$amountbefor = M('Member')->where("id='{$pidinfo['uid']}'")->getField('balance');
				M('member')->where("id='{$pidinfo['uid']}'")->setDec('balance',$pidinfo['amount']);
				$fuddetaildata = [];
				$fuddetaildata['trano']      = $trano;
				$fuddetaildata['uid']      = $pidinfo['uid'];
				$fuddetaildata['username'] =  $pidinfo['username'];
				$fuddetaildata['type']     = 'yongjinshenhe';
				$fuddetaildata['typename']     = '会员撤单';
				$fuddetaildata['remark']       = '会员撤单扣回返点';
				$fuddetaildata['oddtime']     = NOW_TIME;
				$fuddetaildata['amount']   = $pidinfo['amount'];
				$fuddetaildata['amountbefor']   = $amountbefor;
				$fuddetaildata['amountafter']   = $amountbefor - $pidinfo['amount'];
				// print_r($fuddetaildata);exit();
...
/*			if($userinfo['question'] && is_array($userinfo['question'])){
	...
	function betslisttoday($apiparam=array()){
		...
		foreach($touzhulist as $k=>$v){
			...
			$v['type']=($v['ishemai']>0)?C('fuddetailtypes.hemai'):C('fuddetailtypes.order');
			$touzhulist[$k] = $v;
		}
	...
	function hmlist($apiparam=array()){
		...
		unset($apiparam["data"]);
		$lotteryname = $apiparam['lotteryname'];
		$page = ($apiparam['Pageno'])>0?intval($apiparam['Pageno']):1;
		$pagesize = (intval($apiparam['pagesize'])>0 && intval($apiparam['pagesize'])<=30)?intval($apiparam['pagesize']):10;		
		$lotteryinfo = M('caipiao')->where(['name'=>$lotteryname])->field('id,issys,listorder',true)->find();	

		$allsql="and a.cpname='{$lotteryname}'";
		if(empty($lotteryinfo) && $lotteryname!='all'){
			return [];
		}
		$map = [];
		...
		$limit=$GridPage.','.$pagesize;				
		$sql = "select a.id,a.cptitle as lotteryname,a.trano as lotteryid,a.trano as lotteryid,a.username,a.amount as money,a.isfull as ssy,a.fenshu,a.jindu as jdbfb,a.bdjindu as bdbfb,a.hemaipic as onemoney,a.isdraw as state,b.face,a.playtitle,a.oddtime,a.fenshu,b.record from {$DB_PREFIX}touzhu as a left join {$DB_PREFIX}member as b on a.username = b.username where a.ishemai=1 {$allsql} order by a.id desc limit {$limit}";
		
		$touzhulist = M()->query($sql);
		
		foreach($touzhulist as $ks=>$vs){
			foreach($vs as $k=>$v){
				if($k=="username"){
				$touzhulist[$ks][$k]= mb_substr($v,0,2,'utf-8')."*****";
			}
			}
		}
		...
		$apiparam['records'] = $records;
		$apiparam['root'] = $touzhulist;
		$apiparamhm=[$touzhulist,array('pagesize'=>$pagesize,'pageindex'=>$page,'countrs'=>$records,'countpage'=>$totalsize,'page'=>$page,'GridPage'=>$GridPage)];		
		return $apiparamhm;		
	}
