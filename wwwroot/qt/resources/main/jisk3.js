...
var lastMoney = 0.00;
  ...
  function countFun(){
    //二不同号
    var numberarray1 = [];
    $(".games-ethdx-1 .selectNumbers:eq(0) .selectNumber").each(function(){
      var $node = $(this);
    ...
    var numberarray2 = [];
    $(".games-ethdx-1 .selectNumbers:eq(1) .selectNumber").each(function(){
      var $node = $(this);
      if($node.hasClass("curr")){
        var number = $node.attr('ball-number');
    ...
    switch(lottery_code){
      case 'k3hzzx': //和值
      zhushus.length = currNumber[0].length + currNumber[1].length;
      break;
      case 'k3sthdx': //三同号单选
      ...
      break;
      case 'k3sbthbz': //三不同号
      zhushus.length = combines(currNumber[4],3).length;
      break;
      case 'k3slhdx': //三连号单选
      zhushus.length = currNumber[5].length;
      break;
      case 'k3slhtx': //三连号通选
      zhushus.length = currNumber[6].length;
      break;
      case 'k3ethdx': //二同号单选
      ...
      break;
      case 'k3ebthbz': //二不同号
      zhushus.length = combines(currNumber[9],2).length;
      break;
      case 'k3hhm': //红黑码
...
$(function(){
  ...
  $(document).on("click", "#bet_filter span", function() {
    $(this).addClass('ssc-curr').siblings('span').removeClass('ssc-curr');
...
//号码点击
/*$('.g_Number_Section').on('click','.ball_number',function (){
  currNumber = currList();
  countFun()
  countMoney();
})*/
...
var k3hzzx=function(obj){
  ...
  countFun()
  countMoney();
  $(document).on("click", ".addtobetbtn", function() {
    if(currNumber[0].length>0){
...
function setytotal_money(){
  ...
  $('.orderprice').each(function (i) {
    ...
    //ytotal_money = isNaN(parseInt($('#betting_money').eq(i).text()))?0:parseInt($('#betting_money').eq(i).text());
...
//和值
$(document).on("click", ".k3hzzx .selectNumber", function() {
  k3hzzx($(this));
});
var k3hzzx=function(obj){
  if(rates==null){
...
var k3hzzx_addbtn = function(playid){
  $(".k3hzzx .selectNumber.curr").each(function(){
    var obj = $(this);
    var text = obj.text(),number = obj.attr('ball-number'),playid = obj.attr('playid');
...
//三同号通选
$(document).on("click", ".games-sthtx-1 .selectNumber", function() {
  k3sthtx($(this));
});
...
var k3sthtx_addbtn = function(playid){
  $(".games-sthtx-1 .selectNumber.curr").each(function(){
    var obj = $(this);
    var text = obj.text(),number = obj.attr('ball-number'),playid = obj.attr('playid');
    addtotouzhu(playid,number,1);
    setytotal_money();
  });
  $(".games-sthtx-1 .curr").removeClass('curr');
}

//三连号通选
$(document).on("click", ".games-slhtx-1 .selectNumber", function() {
  k3slhtx($(this));
});
...
var k3slhtx_addbtn = function(playid){
  $(".games-slhtx-1 .selectNumber.curr").each(function(){
    var obj = $(this);
    var text = obj.text(),number = obj.attr('ball-number'),playid = obj.attr('playid');
    addtotouzhu(playid,number,1);
    setytotal_money();
  });
  $(".games-slhtx-1 .curr").removeClass('curr');
}
//三同号单选
$(document).on("click", ".games-sthdx-1 .selectNumber", function() {
  k3sthdx_1($(this));
});
var k3sthdx_1=function(obj){
  if(rates==null){
    return false;
  }
  var text = obj.text(),number = obj.attr('ball-number'),playid = obj.attr('playid');
  ...
  countFun()
  countMoney();
  $(".addtobetbtn").attr("onclick", "k3sthdx_1_addbtn('"+playid+"')");
  //addtotouzhu(playid,number);
};

//三连号单选
$(document).on("click", ".gaems-slhdx-1 .selectNumber", function() {
  k3sthdx_2($(this));
});
var k3sthdx_2=function(obj){
  if(rates==null){
    return false;
  }
  var text = obj.text(),number = obj.attr('ball-number'),playid = obj.attr('playid');
  ...
  countFun()
  countMoney();
  $(".addtobetbtn").attr("onclick", "k3sthdx_2_addbtn('"+playid+"')");
  //addtotouzhu(playid,number);
};
//二同号复选
$(document).on("click", ".games-ebhfx-1 .selectNumber", function() {
  k3sthdx_3($(this));
});
var k3sthdx_3=function(obj){
  if(rates==null){
    return false;
  }
  var text = obj.text(),number = obj.attr('ball-number'),playid = obj.attr('playid');
  ...
  countFun()
  countMoney();
  $(".addtobetbtn").attr("onclick", "k3sthdx_3_addbtn('"+playid+"')");
  //addtotouzhu(playid,number);
};

//二不同号
$(document).on("click", ".k3ebthbz .selectNumber", function() {
  k3ebthbz($(this));
});
...
//三不同号
$(document).on("click", ".k3sbthbz .selectNumber", function() {
  k3sbthbz($(this));
});
...
//二同号单选
$(document).on("click", ".games-ethdx-1 .selectNumber", function() {
  var ball  = $(this).attr('ball-number');
  var index = $(this).parents(".selectNmuverBox").index();
  if(index==0){
    $(".games-ethdx-1 .selectNumbers:eq(1) .selectNumber").each(function(index){
    ...
    $(".games-ethdx-1 .selectNumbers:eq(0) .selectNumber").each(function(index){
      var number = $(this).attr('ball-number');
...
//三同号单选/确认选号
var k3sthdx_1_addbtn=function(playid){
  var numberarray = [];
  $(".games-sthdx-1 .selectNumber").each(function(){
    var $node = $(this);
    if($node.hasClass("curr")){
      var number = $node.attr('ball-number');
  ...
  $(".games-sthdx-1 .selectNumber").removeClass("curr");
  setytotal_money();
};
//三连号单选/确认选号
var k3sthdx_2_addbtn=function(playid){
  var numberarray = [];
  $(".gaems-slhdx-1 .selectNumber").each(function(){
    var $node = $(this);
    if($node.hasClass("curr")){
      var number = $node.attr('ball-number');
      numberarray.push(number);
  ...
  $(".gaems-slhdx-1 .selectNumber").removeClass("curr");
  setytotal_money();
};
//二同号复选/确认选号
var k3sthdx_3_addbtn=function(playid){
  var numberarray = [];
  $(".games-ebhfx-1 .selectNumber").each(function(){
    var $node = $(this);
    if($node.hasClass("curr")){
      var number = $node.attr('ball-number');
      numberarray.push(number);
  ...
  $(".games-ebhfx-1 .selectNumber").removeClass("curr");
  setytotal_money();
};
//二不同号/确认选号
var k3ebthbz_addbtn=function(playid){
  ...
  var numberarray = [];
  $(".k3ebthbz .selectNumber").each(function(){
    var $node = $(this);
    if($node.hasClass("curr")){
      var number = $node.attr('ball-number');
      numberarray.push(number);
...
var k3hhm=function(obj){
  ...
  /*if(obj.hasClass('curr');){
    $(".k3hhm .selectNumber").removeClass('curr');
    obj.toggleClass('curr');
  }else{
...
$(document).on("click", ".k3hhm .selectNumberSum", function() {
...
var addtotouzhu = function(_playid,number,zhushu,isshow){
  ...
  //   var _thisType = '['+rate.title+']';
  //   var _thisRmb = $(this).find('.rmb').text();
  //   if(gameNumber == number && _thisRmb == selectRmbStr && gameNumberType == _thisType){
  //     var _thisTimes =  parseInt($(this).find('.yBettingTimess').text()) + parseInt(times);
  //     $(this).find('.yBettingTimess').text(_thisTimes);
  //     $(this).find('#betting_money').text(times * selectRmbStr);
  //     orderList[i].beishu = _thisTimes;
  //     orderList[i].jine = times * selectRmbStr;
  //   }
  // })
  ...
  // html += '<ul>';
  // html += '<li>每注<input class="orderprice" value="'+parseInt(rate.minxf)+'" size="6" style="color:black;" onblur="changeorderprice(this.value,\''+trano+'\');" onafterpaste="formatIntVal(this);changebetokmoney(this.value,\''+rate.rate+'\',\''+trano+'\');" onkeyup="formatIntVal(this);changebetokmoney(this.value,\''+rate.rate+'\',\''+trano+'\');" maxlength="6">元</li>';
  ...
  //if(lottery_code == 'k3hzzx' || lottery_code == 'k3sthtx' || lottery_code == 'k3slhtx'){
...
var delOrder=function(id){
  ...
  if(playid.substring(0,4)=='k3hz')$(".bet-num-box .selectNumber[playid='"+playid+"']").removeClass('curr');
  setytotal_money();
}
...
$(document).on("click", "#f_submit_order", function() {
  ...
  for (var i = 0; i < orderList.length; i++) {
    ...
    if(Number(cur_order.zhushu)<0 || Number(cur_order.price)<0){
...
var lotterytimes = function(lotteryname){
	...
	$.post(url, {'lotteryname':lotteryname/*,'cptype':'ssc'*/}, function(data) {
		if(data.sign==true){
			...
			openLotteryList = $('#openNum_list').find('span');
...
var lotteryopencodes = function(lotteryname){
	...
	$node.html('');
	$.post(url, {'lotteryname':'jisk3'}, function(data) {
		if(data.sign==true){
			...
			for(var o in lotlist){
				...
				html += '<div class="lotterys"><div class="rq">'+openinfo.expect+'</div><div class="hm"><span class="qiu">'+parseInt(array[0])+'</span><span class="qiu">'+parseInt(array[1])+'</span><span class="qiu">'+parseInt(array[2])+'</span></div><div class="k3hz"><em class="k3hz_1">'+sum+'</em><em class="k3hz_2">'+smallbig+'</em><em class="k3hz_3">'+oddeven+'</em></div></div>';
...
var lotteryrates = function(){
	...
	$.each(rates,function(n,value){
		...
		//if(_playid.indexOf('k3hz')==0){
			if(_playid == 'k3hzsmalleven'){
				console.log('k3hzsmalleven');
...
var loadopencode = function(lotteryname){
	...
	clearTimeout(openCodeTimeOut);
	 $.ajax({  
         type:'post',      
         url:url,  
         data:{'lotteryname':'jisk3','expect':openexpect},  
         cache:false,  
         dataType:'json',  
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect>0) {
						...
						html += '</div><div class="k3hz"><em class="k3hz_1">'+sum+'</em><em class="k3hz_2">'+smallbig+'</em><em class="k3hz_3">'+oddeven+'</em></div></div>';
...
//			}
	...
	if (nums.length >= 3) {
		setTimeout(function() {
			var openLotteryList = $('#openNum_list').find('span');
			clearInterval(T3);
