...
function index_list_tag_info(openinfo,index){
	...
	taginfohtml += '</span>';
	taginfohtml += '<input class="btn radius bg_red" value="立即投注" type="button" onclick="openMenuUrl(\''+host+'/lotterys/k3/'+openinfo.name+'\',true)">';
	taginfohtml += '</div>';
...
function index_cplist(lotterylist){ 
	...
	for(var o in lotterylist){
		...
		if(sum%2!=0)
			...
			oddeven='双';
		html += '<li>';
		html += '<a href="javascript:void(0);" onclick="openMenuUrl(\''+host+'/lotterys/k3/'+lotterylist[o].name+'\',true)">';
		html += '<dl>';
		html += '<dt>';
		...
		html += '<div><a class="bg_red" onclick="openMenuUrl(\''+host+'/lotterys/k3/'+lotterylist[o].name+'\',true)">立即投注</a><a class="bg_org" onclick="openMenuUrl(\''+host+'/Game.trend?code='+lotterylist[o].name+'\',false)">走势详情</a></div>';
		html += '</div>';
		html += '</li>';
	};
	$node.html(html);
