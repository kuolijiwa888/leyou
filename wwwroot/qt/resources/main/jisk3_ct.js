...
$(function(){
	...
	if(theRequest.isct) isct = 1;
	if(lotteryname!='undefined'){ 
		Gameinit(lotteryname,isct);
	}else{
		lotteryname = 'f1k3';
		Gameinit(lotteryname,isct);
	};
...
var addtotouzhu = function(_playid,number,zhushu,isshow,minxf){
  if(rates==null){
    return false;
  }
...
var lotterytimes = function(lotteryname){
	...
	$.post(url, {'lotteryname':'jisk3'/*,'cptype':'ssc'*/}, function(data) {
		if(data.sign==true){
			lottery = data.data;
...
var Gameinit = function (_lotteryname,isct) {
	clearInterval(openCodeTimeOut);
	//alert(window.location.pathname);
	//history.replaceState('', '', (host + window.location.pathname +'?code='+ lotteryname));
	if(isct){
	//	History.pushState( '', '', "?code="+_lotteryname+"&isct="+isct);
	}else{
