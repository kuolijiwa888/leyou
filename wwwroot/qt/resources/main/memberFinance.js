				...
				$.each(data.list, function(index, val) {
					if (val.isOpenSys) {
						if (val.isOpen) {
							...
							$("#"+val.value).text("￥"+parseFloat(val.balance).toFixed(3)+"元");
						} else {
							$("#"+val.value).text("未开通");
						}
					} else {
	...
	jQuery.ajax({
		...
		success: function(data, textStatus, xhr) {
			if (data.sign) {
				...
				way.set("user.baccaratRebate", (data.baccaratRebate * 100).toFixed(3));
				way.set("user.peopleBackWater", (data.peopleBackWater * 100).toFixed(3));
				way.set("user.slotBackWater", (data.slotBackWater * 100).toFixed(3));
				way.set("user.sportBackWater", (data.sportBackWater * 100).toFixed(3));
