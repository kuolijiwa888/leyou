$(function () {
  ...
  function updateMaxJJ() {
    ...
    for(var j = 0; j < MaxJJ.length; j++){
      newnewmaxJJs = parseFloat(MaxJJ[j]).toFixed(3);
      if(j == MaxJJ.length - 1){
  ...
  $(document).on("click", "#f_submit_order", function() {
    ...
    for (var i = 0; i < orderList.length; i++) {
        ...
        Orderdetaillist +="<p>"+rate.title+':<span class="mark">'+cur_number+'</span>&nbsp;&nbsp;注数:<span class="mark">'+cur_order.zhushu+'</span>&nbsp;&nbsp;金额:<span class="mark">'+oprice.toFixed(3)+"</span></p>";
    }
