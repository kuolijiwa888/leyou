$(function () {
  ...
  function tabGameInit(){
    ...
    .html('从冠、亚、季、四、五、六、七、八、九、十任意位置上任意选择一个或一个以上号码。');
  ...
  function cqwCount(){
    combineArrUpdataWu();
    //console.log(selected_f_ball_array);
    //console.log(selected_s_ball_array);
    //console.log(selected_t_ball_array);
    //console.log(selected_si_ball_array);
    //console.log(selected_wu_ball_array);
    var itemcount = 0;
      ...
      if(bool){
        if(rate.playid == 'pk10gyhz'){
          ...
          for (var i in Numbersh) {
            ...
            orderList.push(arr);
            winningMoneys = rate_val*times*selectRmb;
            ...
            '</a>'+
            '</div>'+
            '<div id="betting_money" style="display: none;">'+(lastMoney/Numbersh.length)+'</div>'+
            '</dd>';
  ...
  $(document).on('click','#j_play_select li',function () {
    ...
    switch(this_attr){
      ...
      /*case 'gyhz': //jarde gyhz
        ...
        var htm = '<ul>';
        htm += '<li><a class="ball_number selectNumber" playid="pk10gyhd" ball-number="和大" href="javascript:void(0)">大</a><span class="ball_aid" rate_k3hz3="">赔率198.00</span></li>';
        htm += '<li><a class="ball_number selectNumber" playid="pk10gyhx" ball-number="和小" href="javascript:void(0)">小</a><span class="ball_aid" rate_k3hz3="">赔率198.00</span></li>';
        htm += '<li><a class="ball_number selectNumber" playid="pk10gyhdd" ball-number="和单" href="javascript:void(0)">单</a><span class="ball_aid" rate_k3hz3="">赔率198.00</span></li>';
        htm += '<li><a class="ball_number selectNumber" playid="pk10gyhss" ball-number="和双" href="javascript:void(0)">双</a><span class="ball_aid" rate_k3hz3="">赔率198.00</span></li>';
        htm += '</ul>';
        $(".g_Number_Section").html(htm);
        break;*/
        case 'gyhz':
        $('#bet_filter').remove();
        ...
        rates = yrates[_thisPlayid];
        $('.play_select_prompt').find('span[way-data="tabDoc"]')
        .html('选择1个号码组成一注，<span class="moneyInfo">奖金详情</span>');
        var maxjjval = [];
        for (var i = 3; i < 20; i++) {
          maxjjval.push(yrates['pk10gyhz'+i].maxjj);
        }
        maxjjval = maxjjval.join('|');
        addMoneyInfo(bjpks_hzs,maxjjval);
        gameNumber(['和值'],19,3);
        break;
        case 'dxds':
        $('#bet_filter').remove();
        gameSwitch($('.bet_filter_box'),['大小单双'],[{'pk10dxds':'复式'}]);
