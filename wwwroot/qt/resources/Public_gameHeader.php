...
<header class="header" style="background:#000;height:40px;line-height:40px;">
    <div class="container claerfix">
        ...
        <div class="pull-left backLeft" style="color:#fff;">
            ...
            <div class="backLeftLottery" style="display: none;">
                ...
                <div class="betMoreListMenu">
                    <div class="ltBtn active">官方</div>
                    <div class="ltBtn ctbtn">传统</div>
                </div>
                <script>
                    $(function(){
                        $('.betMoreListMenu div').on('click',function(){
                            var that = $(this);
                            console.log(that)
                            that.addClass('active').siblings().removeClass('active');

                            if (that.context.innerHTML == '官方') {
                                $('.backLeftLottery .betMoreList.gfList').css('display','block');
                                $('.backLeftLottery .betMoreList.ctList').css('display','none');
                            }else if (that.context.innerHTML == '传统') {
                                $('.backLeftLottery .betMoreList.ctList').css('display','block');
                                $('.backLeftLottery .betMoreList.gfList').css('display','none');
                            }
                        })
                    })
                </script>
                <div class="betMoreList gfList">
                    <dl class="aLotteryList">
                        ...
                        <dd class="aLotteryListK3">
                            <volist name="Allcp" id="cp" key="key">
                                ...
                                <a href="/lotterys/{$cp.typeid}/{$cp.name}">{$cp.title}</a>
                                </if>
                    ...
                    <dl class="aLotteryList">
                        <dt class="aLotteryListTitle">PK10</dt>
                        <dd class="aLotteryListKLC">
                            <volist name="Allcp" id="cp" key="key">
                                <if condition="$cp['typeid'] eq 'pk10'"><!-- $cp['typeid'] eq 'keno' or  -->
                                    <a href="/lotterys/{$cp.typeid}/{$cp.name}">{$cp.title}</a>
                                </if>
                            </volist>
                        </dd>
                ...
                <div class="betMoreList ctList" style="display: none;">
                    <dl class="aLotteryList">
                        <dt class="aLotteryListTitle" >快三</dt>
                        <dd class="aLotteryListK3">
                            <volist name="Allcp" id="cp" key="key">
                                <if condition="$cp['isct'] eq 1">
                                <if condition="$cp['typeid'] eq 'k3'">
                                <a href="/lotterys_hot/{$cp.typeid}/{$cp.name}/1">{$cp.title}</a>
                                </if>
                                </if>
                            </volist>
                        </dd>
        ...
        <notempty name="userinfo.username">
            ...
            <div class="media-footer">
                <volist name="Think.session.k3names" id="value">
                    <a href="/lotterys/k3/{$value.cpname}" title="{$value.cptitle}" class="color_res" style="font-size:5px;"><span style="color:#333;display: block;margin-top:4px;">{$value.cptitle|substr=0,6}</span><i class="iconfont">&#xe607;</i></a>
                </volist>
            </div></div></div>'>
    ...
    <li class="user_login_info2">
        <a href="/memberCenter/securityCenter" class="my_account" style="color:#fff;">
            我的账户
            <i class="iconfont">&#xe6a1;</i>
        </a>
    ...
    <li class="user_login_info4">
        <a href="/payment/ebankPay" style="color:#fff;">充值</a>
    </li>
    <li class="user_login_info5">
        <a href="/userCenter/withdrawals" style="color:#fff;">提现</a>
    </li>
    <li class="user_login_info6">
        <a href="/loginout" style="color:#fff;">退出</a>
    </li>
    <li>
    ...
    <else/>
    <div class="pull-right user_login_info ">
        <a style="margin:0;color:#fff;" href="/login">亲，请登录</a>
        <em style="margin:0 3px;color:#fff;">|</em> <a href="/register" style="color:#fff;">用户注册</a><em style="margin:0 3px;color:#fff;">|</em>
        <a href="/apublic')}" style="color:#fff;">代理中心</a>
    </div>
    </notempty>
    </div>
</header>
