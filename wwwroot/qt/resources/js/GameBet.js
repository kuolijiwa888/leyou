    ...
    ruiec_GameBet.prototype = {
		...
		*/
        ...
        Change_tzPrice: function () {
            ...
            var _tz_price = isNaN(Number(_tz_len * _tz_beishu * _tz_danwei * 2)) ? 0.00 : parseFloat(Number(_tz_len * _tz_beishu * _tz_danwei * 2)).toFixed(3); //投注的金额
            ...
            $("#choice_Unit").change(function () {
                ...
                if(fdArray)
                {
	                ...
	                for(var a =0; a<fdArray.length;a++)
	                {
	                    ...
	                    //oPtion_val_money = oPtion_val_money.toFixed(3);
        ...
        */
        ...
        ruiec_orderCountMoney: function () {
            ...
            orderCount_money = orderCount_money.toFixed(3);
        ...
        */
        ...
        ruiec_getMyProject: function (oAjaxUrl) {
            ...
            //                var normal_money = isNaN(parseFloat(oData[a].normal_money).toFixed(3)) ? "0.00" : parseFloat(oData[a].normal_money).toFixed(3); //方案金额
        ...
        */
        check_hemaiPrice: function (OrderPrice, theValue) {
            var theEachMoney = parseFloat(OrderPrice / theValue).toFixed(3);  //遍历当前份数下的每份的金额 
            if (theEachMoney >= 0.2) {
                    ...
                    if (typeof theEachMoneyStrArray == "object" && theEachMoneyStrArray.length > 1) {
                        ...
                        if (theXiaoshuLen > 2) {  //小数位的长度大于二
                            ...
                            if (theEndValue > 0) {
                                ...
                                $("#EachMoney").empty().text(parseFloat(theEachMoney).toFixed(3));
                            }
                        } else {
                            $("#hemai_fen").val(theValue);
        ...
        Tongji_HmOrder: function (OrderPrice) {
            ...
            if (theOrderPrice <= 0) {
                ...
                //自己购买需要支付的金额
                var theUserBuyMoney = parseFloat(theUserBuyNum * theEachMoney).toFixed(3);
                //合买保底的份数
                var theBaoDiNum = isNaN(parseInt($("#hemai_baodi").val())) ? 1 : parseInt($("#hemai_baodi").val());
                //合卖保底的金额 
                var theBaodiMoney = parseFloat(theBaoDiNum * theEachMoney).toFixed(3);
                //合买的总金额
                ...
                theHeMaiMoney = parseFloat(theHeMaiMoney).toFixed(3);
